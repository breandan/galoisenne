...
open class RTLGate(
  ...
  companion object {
    fun wrap(value: Any, builder: RTLBuilder? = null): RTLGate =
      if (value is RTLGate) value.mostRecentInstance() else RTLGate(value.toString(), builder)
  ...
  fun asCircuit(
    l: String = neighbors.firstOrNull()?.asCircuit() ?: "",
    r: String = neighbors.lastOrNull()?.asCircuit() ?: "",
    i: String = neighbors.firstOrNull { it.id.toIntOrNull() != null }?.id ?: ""
  ): String = when (op) {
  ...
  open operator fun setValue(builder: RTLBuilder, prop: KProperty<*>, value: Any) {
    ...
    builder.vhdl += (value as RTLGate).asCircuit().lines().let {
      if (it.size > 1) it.mapIndexed { i, l -> "${prop.name}$i := $l;" }.joinToString("\n", "\n", "\n")
