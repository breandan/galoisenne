...
%      \jointspacing
      ...
      \begin{itemize}
        ...
        \item We can encode using the characteristic function, i.e., $\mathds{1}_{\subseteq V}: 2^V\rightarrow \mathbb{Z}_2^{|V|}$
...
%      \jointspacing
      ...
      \begin{center}
        \begin{tabular}{ccccccc}
          ...
          $x \otimes z = \big\{\;w \mid (w \rightarrow x z)\;\big\}$ &&&
...
%        \end{minipage}
      ...
      {\huge
      \hspace*{2.5cm}\begin{minipage}[c]{0.90\columnwidth}
       \vspace{1cm}It is well-known that the family of CFLs is not closed under intersection. For example, consider $\mathcal{L}_\cap := \mathcal{L}(\mathcal{G}_1) \cap \mathcal{L}(\mathcal{G}_2)$ defined in the following way:\\
       ...
       \noindent As $c \rightarrow \infty$, this shape approximates a circular cone whose symmetric axis intersects orthonormal CNF unit productions $w_i \rightarrow t$, with $S_i \in \Lambda^*_{\sigma}?$ encoded by bitvectors on the base perimeter. Equations of this form are equiexpressive with the family of CSLs realizable by finite CFL intersection.
      \end{minipage}
      }
