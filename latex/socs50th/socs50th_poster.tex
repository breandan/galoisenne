...
\begin{document}
  \begin{poster}
    ...
    \begin{multicols}{2}
      ...
      \vspace{1cm}\null\hspace*{3cm}\begin{minipage}[c]{0.90\columnwidth}
        the recognizer is then defined as: $R(\mathcal{G}', \sigma) := S \in V^*? \Longleftrightarrow \sigma \in \mathcal{L}(\mathcal{G})?$
      \end{minipage}
      }
...
%        \end{minipage}
      ...
      \hspace*{2.5cm}\begin{minipage}[c]{0.90\columnwidth}
       ...
       \noindent As $c \rightarrow \infty$, this shape approximates a circular cone whose symmetric axis intersects orthonormal CNF unit productions $w_i \rightarrow t$, with $S_i \in V^*_{i}?$ encoded by bitvectors on the base perimeter. Equations of this form are equiexpressive with the family of CSLs realizable by finite CFL intersection.
      \end{minipage}
      }
