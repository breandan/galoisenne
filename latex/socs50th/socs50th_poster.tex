...
%      \jointspacing
      ...
      \null\hspace*{3cm}\begin{minipage}[c]{0.90\columnwidth}
      \begin{itemize}
        \item CYK parsers can be lowered onto $\mathbb{Z}_2^{n\times n \times |V|}$ or $\mathcal{M}: (\mathbb{Z}_2^p\rightarrow\mathbb{Z}_2)^{n\times n \times|V|}$
        ...
        \item $\oplus, \otimes$ are defined as $\highlight{\boxplus}, \highlight{\boxtimes}$, so that the following diagram commutes:
      \end{itemize}
      \vspace{2cm}
...
%    \end{pmatrix}
      ...
      \hspace*{2.5cm}\begin{minipage}[c]{0.90\columnwidth}
       ...
       \noindent As $c \rightarrow \infty$, this shape takes the form of a circular cone whose symmetric axis intersects orthogonal CNF unit productions $w_i \rightarrow t$, with $S_i \in \mathcal{T}_{i}$ encoded by bitvectors along the base perimeter. Equations of this form are equiexpressive with the family of CSLs realizable by finite CFL intersection.\\
