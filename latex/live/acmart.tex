...
In the following benchmark, we analyze bracketing errors in a dataset of natural code snippets in Java, Kotlin and Python. For Java and Kotlin, we sample valid single-line statements on GitHub with a nontrivial bracket nesting, synthetically delete $1\ldots d$ brackets uniformly at random, repair using Tidyparse, take the top-1 repair after $t$ seconds, and measure compiler acceptance.
...
For Python, we sample invalid Python code snippets uniformly from the Break-It-Fix-It (BIFI) dataset~\cite{yasunaga2021break}, a dataset of human errors in natural Python snippets, repair them using Tidyparse, take the top-1 repair after $t$ seconds, and measure interpreter acceptance.
...
%Although not intended to be a dedicated parser and we make no attempt to rigorously compare parsing latency, parsing valid strings with Tidyparse is typically competitive with classical parsing methods.
We envision three primary use cases: (1) helping novice programmers become more quickly familiar with a new programming language (2) autocorrecting common typos among proficient but forgetful programmers and (3) as a prototyping tool for PL designers and educators.
...
From a practical standpoint, we have implemented our approach as an IDE plugin and demonstrated its viability as a tool for live programming. Tidyparse is capable of generating repairs for invalid code in a range of toy languages. We plan to continue expanding its grammar and autocorrection functionality to cover a broader range of languages and hope to conduct a more thorough user study to validate its effectiveness in the near future. Further examples can be found at: \url{https://tidyparse.github.io}
%\begin{itemize}
