...
\end{verbatim}
\hspace{0.63cm}\emoji{herb}\hspace{4.02cm}\emoji{herb}\vspace{-17pt}
\begin{verbatim}
    └── if [0]                └── then [6]
\end{verbatim}
\hspace{0.63cm}\emoji{herb}\hspace{4.02cm}\emoji{herb}\vspace{-17pt}
...
\varnothing      & \cdd &      &      &      &      &      &      & \varnothing &
  \end{NiceMatrix}
\]
    \caption{Mountains in the ridge structure correspond to parseable substrings. We iterate over upper diagonals in decreasing altitude and discard any subtree that falls totally within the shadow of another. The example shown above has three branches, rooted at nonterminals $C, B, A$.}
\end{figure}
