...
Prior work in this area follows two main streams. Kats~\cite{kats2009providing} and deJong~\cite{de2012automated} and Diekmann~\cite{diekmann2018dont} investigate error repair using LR grammars and more recently, Raselimo and Fischer explore repair in the grammar itself~\cite{raselimo2021automatic}. Our approach can handle a more general class of context-free and bounded context-sensitive grammars and has a more theoretically rigorous grounding in language reachability~\cite{melski1997interconvertbility}. Consequently, it is much simpler and can scaled up using a GPU. Tradeoffs exist as usual, which are discussed in Sec.~\ref{sec:discussion}.
...
%\begin{verbatim}
% F.( → (   S → S S     F.λ → λ   v...S → F.v ..S
% F.) → )   S → c        ε+ → ε       S → F.( S.)
% F.. → .   S → v        ε+ → ε+ ε+   S → F.λ v...S
% F.v → v   S → S ε+    ..S → F.. S
% F.ε → ε   S → S F.ε   S.) → S F.)
%\end{verbatim}
...
\noindent where $\mathcal{V}$ is a function $\mathbb{Z}_2^{|V|}\rightarrow\mathbb{Z}_2$. Note that while it is always possible to encode $\mathbb{Z}_2^{|V|} \rightarrow \mathcal{V}$ using the identity function, the corresponding decoder $(\varphi^{-1})$ is not guaranteed to exist, since an arbitrary bitvector expression $(\mathcal{V})$ might take on zero, one, or in general, multiple solutions in $\mathbb{Z}_2^{|V|}$.
...
\noindent Depicted in (\ref{eq:satten}) is a SAT tensor representing \hlgray{$\sigma_1\:\sigma_2\:\sigma_3$}$\:\_\:\ldots\:\_$ where shaded regions demarcate known bitvector literals $\mathcal{L}_{r,c}$ (i.e., representing established nonterminal forests) and unshaded regions correspond to bitvector variables $\mathcal{V}_{r,c}$ (i.e., representing seeded nonterminal forests to be grown). Since $\mathcal{L}_{r,c}$ are fixed, we precompute them outside the SAT solver.
...
\begin{tidyinput}
(*@true and true \err{! and false or true !} true and false\caret{ }@*)
\end{tidyinput}

\begin{verbatim}
  Parseable subtrees (5 leaves / 2 branches):
\end{verbatim}
\noindent\hspace{0.97cm}\emoji{herb}\hspace{1.69cm}\emoji{herb}\hspace{2cm}\emoji{herb}\vspace{-8pt}
\begin{verbatim}
      └── ! [3]   └── and [4]   └── false [5]
\end{verbatim}
\noindent\hspace{0.97cm}\emoji{herb}\hspace{1.69cm}\emoji{herb}\vspace{-8pt}
\begin{verbatim}
      └── or [6]  └── true [7]
\end{verbatim}
\hspace{0.97cm}\emoji{herb}\hspace{3.38cm}\emoji{herb}\vspace{-8pt}
\begin{verbatim}
      └── S [0..2]          └── S [8..11]
          ├── true [0]          ├── S [8..9]
          ├── and [1]           │   ├── ! [8]
...
\noindent This enables adding a placeholder for typed expressions:
...
\begin{tidyoutput}
false or ! true or (*@\hlorange{true}@*) and <S> or <S>
false or ! true or (*@\hlorange{false}@*) and <S> or <S>
false or ! true or (*@\hlorange{!}@*) (*@\hlorange{<S>}@*) and <S> or <S>
false or ! true or (*@\hlorange{<S>}@*) (*@\hlorange{and}@*) (*@\hlorange{<S>}@*) and <S> or <S>
false or ! true or (*@\hlorange{<S>}@*) (*@\hlorange{or}@*) (*@\hlorange{<S>}@*) and <S> or <S>
...
\end{tidyoutput}
...
\end{tidyinput}
\begin{tidyoutput}
( λ f . ( λ x . f ( x x ) ) (*@\hlorange{)}@*) λ x . f ( x x )
( λ f . ( λ x . f ( x x ) ) (*@\hlorange{x}@*) (*@\hlgreen{)}@*) λ x . f ( x x )
( λ f . ( λ x . f ( x x ) ) ( λ x . f ( x(*@\hlred{ }@*)) (*@\hlgreen{)}@*) (*@\hlgreen{)}@*)
...
\end{tidyoutput}

\subsection{Context-Sensitive Languages}

Many natural and programming languages exhibit context-sensitivity, such as Python indentation. Unlike traditional parser-generators, Tidyparse can encode CFL-intersection, allowing it to detect and correct errors in a broader family of languages than would normally be possible using CFGs alone. For example, consider the grammar from Sec.~\ref{sec:cslr}, which can be translated in the following way:
...
In the following benchmark, we analyze bracketing errors in a dataset of Java and Python code snippets on GitHub. For Java, we sample valid single-line statements with a nontrivial bracket nesting, synthetically delete $1\ldots d$ brackets uniformly at random, repair using Tidyparse, take the top-1 repair after $t$ seconds, and measure compiler acceptance. To repair the snippets, we use the same language as in Sec.~\ref{sec:latency}, replacing all other tokens with \texttt{w}.

\begin{figure}[H]
\centering{\textbf{Synthetic Error Correction}}\\
\vspace{0.25cm}
...
%    title={\hspace{-1cm}\textbf{Java Brackets}},
    ...
    \addplot table [x expr=\coordindex, y=d1]{\naturalerrors};
    \addplot table [x expr=\coordindex, y=d2]{\naturalerrors};
    \addplot table [x expr=\coordindex, y=d3]{\naturalerrors};
    \legend{1s, 10s, 20s}
  \end{axis}
\end{tikzpicture}
...
%    width=8.3cm,
%    height=6.6cm,
%    title={\hspace{-1cm}\textbf{Kotlin Brackets}},
%    ybar,
%    bar width=10pt,
%    xlabel={$|\Sigma|$},
%%    ylabel={Compiler acceptance},
...
For Python, we sample invalid Python code snippets uniformly from the imbalanced bracket category of the Break-It-Fix-It (BIFI) dataset~\cite{yasunaga2021break}, a dataset of human errors in Python snippets, repair them using Tidyparse, take the top-1 repair after $t$ seconds, and measure interpreter acceptance.
...
\hspace{-0.25cm}\begin{tikzpicture}
  \begin{axis}[
    width=8.3cm,
    height=6.6cm,
    title={\hspace{-1cm}\textbf{Natural Error Correction}},
    ybar,
    bar width=10pt,
    xlabel={$|\Sigma|$},
    ylabel={Interpreter acceptance},
    xtick=data,
    axis x line*=bottom,
    axis y line*=left,
    enlarge x limits={abs=0.5},
...
Although not intended to be a dedicated parser and we make no attempt to rigorously compare parsing latency, parsing valid strings with Tidyparse is typically competitive with classical parsing methods. We envision three primary use cases: (1) helping novice programmers become more quickly familiar with a new programming language (2) autocorrecting common typos among proficient but forgetful programmers and (3) as a prototyping tool for PL designers and educators.
