...
Assuming $d$ holes, there are ${n \choose d}$ possible hole configurations, each with $(|\Sigma| + 1)^{2d}$ possible repairs. In practice, depending on $n$ and $d$, this space can be intractable to search through exhaustively, and a uniform distribution may be sample-inefficient, so to facilitate real-time assistance we prioritize likely repairs according to a four-step procedure:

\begin{enumerate}
    \item Fetch the most recent $\mathcal{G}$ and string $\err{\sigma}$ from the editor.
...
Morally, we would prefer sketch templates and repairs that are ``natural'' for an invalid string. To do so, we draw holes and rank admissible repairs using a distance metric over the Levenshtein q-ball, $\Delta_q(\sigma)$. One such metric, the Kantorovich--Rubinstein (KR) metric, can be viewed as an optimal transport problem, $\delta_{KR}$ minimizing over $\Pi$, the set of all mass-conserving transportation plans between two probability distributions $\mu$ and $\nu$ over some metric space $\Omega$:

\begin{align}
    \delta_{\textsc{KR}}(\mu, \nu) := \inf_{\pi\in \Pi(\mu, \nu)}\int_{\Omega\times \Omega} \delta(x, y)d\pi(x, y)
\end{align}

More specifically in our setting, the metric space $\Omega$ factorizes into (1) the specific edit locations (e.g., informed by caret position, historical edit locations, or static analysis), (2) tokenwise completions (e.g., from a Markov or neural language model) and (3) an associated \textit{cost model}, which can be any number of suitable distance metrics, e.g., finger travel distance on a physical keyboard in the case of typo correction, weighted Levenshtein distance for string edits, or stochastic contextual edit distance in the case of tree edits.
