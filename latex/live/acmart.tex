...
Since there are $\Sigma_{d=1}^q{n \choose d}$ sketch templates in total, each with $(|\Sigma| + 1)^{2d}$ possible repairs, if $n$ and $q$ are large, this space can be intractable to search through exhaustively and a uniform prior may be highly sample-inefficient. In order to facilitate real-time assistance, we prioritize likely repairs according to the following four-step procedure:

\begin{enumerate}
    \item Retrieve the most recent $\mathcal{G}$ and $\err{\sigma}$ from the editor.
    \item Lazily enumerate $\stirlingii{n}{d}$ for increasing values of $d \geq 1$.
    \item Draw $\bm{\sigma}\sim\Sigma^{n\pm q}\cap\Delta_{q}(\err{\sigma})$ sans replacement using~\ref{sec:dsi}.
    \item Decode and rerank models by Levenshtein distance.
\end{enumerate}

For example, suppose we have $\err{\sigma}: \Sigma^{90}$, and a probability distribution over all possible edits. This distribution, which we call $\mathcal{F}$, is then used to prioritize admissible repairs. Reprojected onto $\err{\sigma}$, the distribution $\mathcal{F}$ might appear thusly:
