...
\noindent where $\mathcal{V}$ is a function $\mathbb{Z}_2^{|V|}\rightarrow\mathbb{Z}_2$. Note that while always possible to encode $\mathbb{Z}_2^{|V|} \rightarrow \mathcal{V}$ using the identity function, $\varphi^{-1}$ may not exist, as an arbitrary $\mathcal{V}$ might take on zero, one, or in general, multiple solutions in $\mathbb{Z}_2^{|V|}$.
...
For Python, we use the Break-it-fix-it (BIFI) dataset~\cite{yasunaga2021break}, a dataset of human errors in natural Python snippets, repair using Tidyparse, take the top-1 repair after $t$ seconds, and measure interpreter acceptance.

\hspace{-0.5cm}\begin{tikzpicture}
    \begin{axis}[
        width=8.3cm,
        height=6.6cm,
        title={\hspace{-1cm}\textbf{Natural Error Correction (Python Brackets)}},
        ybar,
        bar width=10pt,
        xlabel={$|\Sigma|$},
