...
\pagebreak\section{Realtime Error Correction}\label{sec:holes}

So we have a procedure $P: \mathcal{G} \times \Sigma^d \rightarrow \{\Sigma^d\}$. But where do we put the holes? For a given number of holes, k, there are roughly ${n \choose k}$ possible repairs. In practice the cardinality of this space can be very large. To provide real-time suggestions, we generate the repairs according to a nine-step procedure:
...
\section{Tree Denormalization Procedure}
...
The matrix parser produces a binary tree corresponding to the normalized grammar. To recover the original parse tree, we recursively remove synthetic nodes.
