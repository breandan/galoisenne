...
Assuming $d$ holes, there are ${n \choose d}$ possible hole configurations (HCs), each with $(|\Sigma| + 1)^{2d}$ possible repairs. In practice, depending on $n$ and $d$, this space can be intractable to search through exhaustively, so to facilitate real-time assistance we prioritize likely repairs according to an eight-step procedure:
...
We can think of this procedure as a moment-matching procedure between some joint distribution over the edit locations and an error model (e.g., finger travel distance on a physical keyboard or n-gram probability, in the case of typo correction). Morally, we want to prioritize sketch templates which are nearby probable errors and repairs which are, in some sense, ``natural'' for a given context.
