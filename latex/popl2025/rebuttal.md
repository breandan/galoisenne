...
Everyone seemed to have some confusion about the significance of the porous completion problem, so let's start by clarifying that point, then we will address the specific concerns of each reviewer.

Porous completion is a general technique for abstract parsing with holes that we have found useful for many problems involving CFGs. It is specifically necessary in this paper to construct the parameterized Parikh map, as we explain in 4.6, equation (10).
...
What this sentence basically says is that we precompute $\mathbb{T}_3$ for porous strings up to some large number, i.e, {`_`, `_ _`, `_ _ _`, ...}. We abstractly parse the porous strings, then count the minimum and maximum number of each terminal that each nonterminal can generate, assuming its length falls between $m$ and $n$. This can be computed once for each grammar and cached for later use in the modified Bar-Hillel construction.

Suppose we are given a broken string of length |σ| = 5, and we want to search a Levenshtein radius of d = 2. Then we would look up the parameterized Parikh map between length 3 and 7, representing the minimum and maximum number of each terminal that each nonterminal can generate, across all parsable strings between length 3 and 7. To avoid enumerating all possible strings, we use the idempotent matrix completion technique to construct $\mathbb{T}_3$, then just store $\pi(G, v, m, n)$ for later use.
...
See our response to Reviewer #257C (What "fraction"?) for a back-of-the-envelope calculation of our method's energy consumption relative to Seq2Parse and BIFI.
...
Diekmann and Tratt's paper does handle arbitrary CFGs, nor guarantee completeness across all possible repairs in all locations, only a fixed location. It is designed to work only with LR parsers. It could be a good baseline to compare against for Python.
...
The techniques are designed to be executed on fundamentally different computing platforms – in fact, we even gave an advantage to the neural repair models by evaluating on GPUs which have significantly more processing power. Please see our reply to Reviewer #257C (What "fraction"?) for a more detailed comparison of the energy consumption of our method relative to Seq2Parse and BIFI.
...
The text should read 30s, although this is the global timeout and each trial does not necessarily use the entire time - indeed, as a function of there being fewer repairs to enumerate. This is the advantage of using a generating function: it gives you a "progress bar" and tells you once there are no further repairs to enumerate.
...
For the purposes of this presentation $\mathbb{T}_2$ is effectively nonparametric, however in practice a parametric type $a$ can be used to propagate other aggregate statistics, such as the PPM or $|T: \mathbb{T}_2|$. We could have equivalently written $P(a) = \Sigma + aVL(V^2 P(a))$.
...
Currently, these constraints are enforced at the value level, by the implementation of $\oplus, \otimes$. If the reviewer knows a way to enforce these constraints at the type level, we would be interested in discussing it further.
...
Absolutely! Datalog and ASP are both more expressive than CFGs, so encoding the problem declaratively is relatively straightforward. Previously, we implemented the problem a number of ways declaratively, but found it to be very slow. One advantage of using Datalog would be to leverage the incremental solvers, instead of implementing a custom incremental solution for realtime typing assistance, which we did not describe in this paper.
...
Another downside of declarative approaches is there are relatively few parallel solvers. We rely heavily on parallelism to make the problem tractable, so we would need to investigate how to parallelize Datalog or ASP solvers to make them competitive. We also want the flexibility to define probability distributions over formal languages and customize the sampling order. Fusing Probabilistic sampling with Datalog / ASP is still an active research problem, so we opted for imperative + parallel as the best fit for our problem, giving us the most implementation flexibility.
...
- **Nominal** refers to the theory of universals originating from 13th century theologian and logician William of Occham, the founder of nominalism. More recently, it was taken up in the context of programming languages by Gabbay and Pitts under the auspices of "nominal sets" to refer to Fraenkel-Mostowski set theory, then adopted by Bojańczyk et al. who developed a theory of automata based on nominal sets. In the context of this paper, it denotes an automata with arcs that accept or reject a terminal. A minor variant is studied under the term "symbolic automata" by D'Antoni et al. (2014, 2017), however, we prefer the nominal formalism as it is explicitly designed to handle variable naming semantics in a programming language. In this particular paper, nominalization effectively reduces the number of arcs in the Levenshtein automaton.
