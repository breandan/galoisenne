...
\documentclass[sigplan,review,acmsmall,nonacm,anonymous]{acmart}\settopmatter{printfolios=false,printccs=false,printacmref=false}
...
\State $\mathcal{R} \gets \varnothing, \texttt{seed} \gets 0$ \Comment{Initialize set of parse trees.}
...
\State $\mathcal{R} \gets \mathcal{R} + \varphi(T, i)$ \Comment{Decode fresh parse tree and add to retrieved set.}
%\Until{interrupted or $\texttt{seed} = |T|$.}
\EndFor
\State \Return $\mathcal{R} \textbf{ ranked by } NLL\big(\cdot \Rightarrow \Sigma^* \big)$ \Comment{Flatten trees and rank by sentence perplexity.}
\end{algorithmic}
\end{algorithm}
