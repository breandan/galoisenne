...
\end{equation}
  ...
  The previous technique will enumerate parse trees in a given $\mathbb{T}_2$, but in an arbitrary order. While sufficient for the purpose of decoding small finite languages, this can be improved for larger languages by sampling repairs in order of decreasing likelihood. First, we observe the following:
...
\alpha \mid \mathcal{L}(\alpha) = \{\texttt{root}(T)\} & T: \Sigma, \\
\sum_{\langle T_1, T_2\rangle \in \texttt{children}(T)} \mathcal{Y}(T_1)*\mathcal{Y}(T_2) & T: VL(V^2P(a)^2)
\end{cases}
\begin{equation}
