...
%  \end{enumerate}
  ...
  We will first describe how we generate the intersection grammar (\S~\ref{sec:lev_nfa},~\ref{sec:lev_bh}), then, define a data structure compactly representing its language, allowing us to efficiently decode all repairs contained within (\S~\ref{sec:ptree}). To do so, one can choose to either enumerate repairs from the CFG (\S~\ref{sec:tree_sampling}), or sample repairs from a DFA to improve the naturalness of the top-k repairs (\S~\ref{sec:decoding}).
