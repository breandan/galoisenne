...
%  \end{enumerate}
  ...
  We present three basic methods for repair decoding: enumerate parse trees from the CFG, $G_\cap$, and rerank each tree by either (1) PCFG score, or (2) Markov chain likelihood, or (3) translate $G_\cap$ to an equivalent DFA, $\mathcal{A}_\cap$, minimize it using Brzozowski's algorithm to produce $\mathcal{A}_\cap^*$, then sample trajectories without replacement through the DFA according to a Markov chain until a fixed timeout is reached. We describe (Alg. 1) and (Alg. 2) in \S~\ref{sec:tree_sampling}, and (Alg. 3) in \S~\ref{sec:decoding}.
  ...
  We will first describe how we generate the intersection grammar (\S~\ref{sec:lev_nfa},~\ref{sec:lev_bh}), then, define a data structure compactly representing its language, allowing us to efficiently decode all repairs contained within (\S~\ref{sec:ptree}). Optionally, we can choose to either enumerate repairs from the CFG, or sample repairs from a DFA to improve the naturalness of the top-k repairs (\S~\ref{sec:ranking}).
