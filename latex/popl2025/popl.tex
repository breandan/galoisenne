...
We will now give an informal intuition behind of our method, then proceed to formalize it in the following sections. By way of illustration, suppose we have a string $\texttt{( ) )}$, and wish to find nearby repairs. There is a nondeterministic finite automaton, called the Levenshtein automaton, recognizing every single string that can be formed by inserting, substituting or deleting a parenthesis. We will use a variant that removes some unnecessary edges, but does not affect the generated language.\vspace{-4pt}
...
\noindent from analytic combinatorics, however unlike Duchon et al.~\cite{duchon2004boltzmann}, our work does not depend on rejection to guarantee exact-size sampling, as all trees from $\mathbb{T}_2\cong\mathcal{G}_\cap'$ are destined to all be within a small Levenshtein distance of each other.

  \begin{algorithm}[H]
    \caption{Ordinary PCFG sampling}
    \label{alg:adaptive}
    \begin{algorithmic}[1]
      \Require $T: \mathbb{T}_2$ intersection grammar, $\Gamma: \mathbb{T}_2 \rightsquigarrow \mathbb{T}$ sampler, $P_\theta: \mathbb{T} \rightarrow \mathbb{R}$ PCFG
...
\Repeat
\State $\hat{A} \gets \hat{A} \cup \big\{\Gamma(T)\big\}$ \Comment{Sample a parse tree and add to retrieved set.}
\Until{interrupted.}
\State \Return $\hat{A} \textbf{ ranked by } P_\theta(\cdot)$ \Comment{Rank by PCFG likelihood.}
\end{algorithmic}
\end{algorithm}

  This sampler is simple and fast, but does not guarantee uniformity over the set of all generable parse trees, and does not converge linearly, failing to satisfy Def.~\ref{def:linear-convergence}. To get around this issue, we will define an alternate sampler based on an integer bijection, sample integers without replacement, decode them into trees, and finally rerank by a Markov chain.
...
\State $t \gets \varphi\big(T, \texttt{RandomIntWoR}(\texttt{seed}, |T|)}\big)$ \Comment{Draw unique $\mathbb{Z}_{|T|}$ and decode into fresh parse tree.}
\State $\hat{A} \gets \hat{A} \cup \{t\}$
%\Until{interrupted or $\texttt{seed} = |T|$.}
\EndFor
\State \Return $\hat{A} \textbf{ ranked by } NLL\big(\text{\textleaf}(t)\big)$ \Comment{Defoliate and rank by n-gram likelihood.}
\end{algorithmic}
\end{algorithm}
...
In practice, this means the tree sampler can produce multiple parse trees which represent the same string, impeding convergence. We can eliminate ambiguity and thereby improve the rate of convergence for natural syntax repair by translating $\mathbb{T}_2$ into a DFA, then sampling repair trajectories in order of decreasing string likelihood. First, let us note the following:
...
\begin{equation}
\text{Precision@k}(R) = \frac{1}{|\mathcal{D}_{\text{test}}|}\sum_{\langle\sigma^\dagger, \sigma'\rangle \in \mathcal{D}_{\text{test}}} \mathds{1}\big[\sigma' \in \argmax_{\bm{\sigma} \subseteq R(\sigma^\dagger), |\bm{\sigma}| \leq k}\sum_{\sigma \in \bm{\sigma}}\text{NLL}(\sigma)\big]
\end{equation}

  This is a variation on a standard metric used in information retrieval, and a common way to measure the quality of ranked results in machine translation and recommender systems. Precision@All or completeness may be seen as a special case, where $k=\infty$.
