...
The literature on parsers is vast and deep, covering far more ground than we could possibly hope to survey. We take inspiration from their findings, but restrict ourselves to a dozen most closely related, in three major research areas: (1) formal language theory, (2) program synthesis, and (3) error correction. We survey each of these areas in turn.

It was Noam Chomsky himself who first introduced the algebraic theory of \textit{context-free grammars} (CFGs) in 1959~\cite{chomsky1959algebraic}, and since then, CFGs have been the subject of extensive research in formal language theory and program analysis. In particular, we take inspiration from Leslie Valiant~\cite{valiant1975general} who first discovered the connection to matrix multiplication in 1975 and Alexander Okhotin~\cite{okhotin2001conjunctive} who later introduced the idea of \textit{conjunctive grammars} in 2001. More recently, Azimov \& Grigorev~\cite{azimov2018cfpq} and Qirun Zhang shed light into the practical applicability of these ideas and made the theory of CFG reachability more accessible to the general public. In our work, we show how to compile their ideas onto a SAT solver to fix syntax errors, which seems like a perfectly natural extension, but was heretofore previously never considered to the best of our knowledge.

In the SAT solving community, there is some interesting work on domain-specific solvers for string constraints like HAMPI~\cite{kiezun2009hampi}, which is based on the idea of \textit{tree automata}. Tree automata are a generalization of finite automata, which can be used to model a variety of string constraints, including regular expressions and certain context-free grammars. There is also some work from Loris D'Antoni on symbolic automata~\cite{dantoni2014minimization}, which are a generalization of finite automata that allow arbitrary symbolic expressions in the transitions. Unlike their work, our solver can handle CFGs and conjunctive grammars in realtime and does not require any special encoding of the grammar. The connection to matrix multiplication makes it particularly appealing for parallelization.
