...
% an abbreviated paper title here
  ...
  \begin{definition}[Generalized Regex]
    ...
    \begin{multicols}{2}
      ...
      \end{eqnarray*}
    \end{multicols}
    \footnotetext{Or $\{a \cdot b \mid a \in \mathcal{L}(G) \land b \in \mathcal{L}(D) \}$ to be more precise.}
  \end{definition}
  ...
  \begin{theorem}[Generation]
    ...
    $\texttt{follow}(R):RE \rightarrow 2^\Sigma$ = \begin{cases}
     \{R\} &\text{ if } R \in \Sigma \\
     \texttt{follow}(G) &\text{ if } R = G \cdot D\\
     \texttt{follow}(G)\cup\texttt{follow}(D) &\text{ if } R = G \lor D
    \end{cases}\\\\

    $\texttt{choose}(R):RE \rightarrow \Sigma^+$ = \begin{cases}
     R &\text{ if } R \in \Sigma \\
     \big(s \sim \texttt{follow}(R)\big)\cdot \texttt{choose}(\partial_sR) &\text{ if } R = G \cdot D\\
     \texttt{choose}(R' \sim \{G, D\}) &\text{ if } R = G \lor D
    \end{cases}
  \end{theorem}
