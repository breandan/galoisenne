...
\begin{theorem}%[Considine, 2025]
  For every CFG, G, and every acyclic NFA (ANFA), A, there exists a decision procedure $\varphi: \text{CFG} \rightarrow \text{ANFA} \rightarrow \mathbb{B}$ such that $\varphi(G, A) \models [\mathcal{L}(G)\cap\mathcal{L}(A) \neq \varnothing]$ which requires $\mathcal{O}\big((\log |Q||V|)^c\big)$ time using $\mathcal{O}\big((|Q||V|)^k\big)$ parallel processors for some $c, k < \infty$.
\end{theorem}

\begin{proof}[Proof]
  We want to show there exists a path $p \rightsquigarrow r$ in A such that $p\in I, r\in F$ where $p \rightsquigarrow r \vdash S$. There are two cases, at least one of which must hold for $w \in V$ to parse a given $p \rightsquigarrow r$ pair:
  ...
  \noindent Note we can check $\varphi$ before each recurrence of $T$ and escape immediately thereafter in case of nonemptiness. Should that occur, to decode a witness one may simply $\texttt{choose}(S_\cap)$ (see Thm.~\ref{thm:generation}).
\end{proof}\clearpage
