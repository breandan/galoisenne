...
In the case of ordinary CFL recognition, the automaton forming a single row that accepts one word:
...
We use the Precision@k statistic, which measures the frequency the true repair appeared in the top-k results, across a dataset of instances. Specifically, given a repair model, $R: \Sigma^* \rightarrow (\Sigma^*)^t$ and a test set, $\mathcal{D}_{\text{test}}$, consisting of pairwise aligned errors ($\err\sigma$) and fixes ($\sigma'$), we define Precision@k as:
...
We compare our method with two external baselines, Seq2Parse and Break-It-Fix-It (BIFI)~\cite{yasunaga2021break} on a single test set. This dataset~\cite{wong2019syntax} consists of 20k naturally-occurring pairs of Python errors and human fixes from StackOverflow and is used to compare the precision of each method at blind recovery of the ground truth repair. We preprocess all source code by filtering for broken-fixed snippet pairs shorter than 80 tokens and fewer than three Levenshtein edits apart. We then balance the dataset by sampling an equal number of repairs from each length and Levenshtein edit distance, resulting in a test set of 1,755 pairs, with approximately 70 repairs in each (length, distance) bin.
...
To train the reranker, we prepare a training set of 10k repair instances from the StackOverflow dataset, where $\mathcal{D}_{\text{train}} \cap \mathcal{D}_{\text{test}} = \varnothing$. Each instance contains a single syntax error, $\err\sigma$, the ground truth repair, $\sigma'$, and up to $10^3$ confounders sampled without replacement from the $\mathcal{L}\bigl(L(\err\sigma, d^* + 1)\bigr)\cap \ell$ intersection using the pretrained $c$-gram model. This model is trained for 20k steps on a standard MacBook Pro, which takes about 24 hours. Hyperparameter settings are provided in Appendix~\ref{sec:hyperparams}.
...
\resizebox{.24\textwidth}{!}{\input{len_dist_tidy_p100}}
\resizebox{.24\textwidth}{!}{\input{len_dist_tidy_p1000}}
...
\clearpage The remaining experiments in this section were run on a 16-core ARM64 M4 Max with 128 GB of memory. We balance the StackOverflow dataset across Levenshtein distances, then measure the number of samples required to draw the exact human repair across varying Levenshtein radii. This tells us of how many samples are required on average to saturate the admissible set.
...
\caption{4-gram repairs. Precision@1000 is an upper bound.}\label{fig:adaptive}
\end{figure}
