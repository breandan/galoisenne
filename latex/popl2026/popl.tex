...
Primarily, this gadget was designed to handle membership queries, for which purpose it has received considerable attention in recent years~\cite{might2011parsing,adams2016complexity,stanford2021symbolic,varatalu2025re}:
...
Recall that since every regular language is context-free. So, given a $(\varepsilon, \land)$-free regular expression, we can construct an equivalent CFG with start symbol \( S_e \) and productions \( P(e) \) as:
...
\begin{proof}[Proof]
  ...
  Now, our goal is to find $M=M^2$ such that $\big[M_0[r, c, w] \neq \varnothing\big] \implies \big[M[r, c, w] \neq \varnothing\big]$ under a certain near-semiring. The algebraic operations $\oplus, \otimes: E^{2|V|} \rightarrow E^{|V|}$ will be defined elementwise:
