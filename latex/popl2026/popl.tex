...
\section{Method}
  ...
  In this section, we will consider three examples of intersections with finite languages. First, parsing can be viewed as a special case of CFL-FSA intersection where the automaton which accepts a single word. Second, completion can be seen as a case of intersection where the hole locations are known, but can be filled. Finally, we will consider the case of repair, in which case we will need to intersect with a langauge representing all possible edits up to a fixed bound.

  \subsection{Parsing as intersection}
  ...
  We can also consider a more general automaton for completing a string with holes, representing missing terminals in known locations. The same procedure can be lifted to the domain of strings containing free variables, which we call the \textit{porous completion problem}. In this case, the fixpoint is characterized by a system of language equations, whose solutions are the set of all sentences consistent with the template.
