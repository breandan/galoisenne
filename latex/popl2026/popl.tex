...
For our first experiment, we measure the precision of our repair procedure at various lengths and Levenshtein distances, comparing Tidyparse against Seq2Parse, vanilla BIFI and BIFI with a beam size and precision at $2\times10^4$ distinct samples.

\begin{figure}[h!]
\resizebox{.31\textwidth}{!}{\input{len_dist_tidy}}
\resizebox{.31\textwidth}{!}{\input{len_dist_s2p}}
\resizebox{.31\textwidth}{!}{\input{len_dist_bifi}}
...
%
\begin{figure}[h!]
\resizebox{.31\textwidth}{!}{\input{len_dist_tidy_p10}}
\resizebox{.31\textwidth}{!}{\input{len_dist_bifi_all}}
...
We can quantify the improvement in rank by comparing the histogram of the true document's rank across the test set, with and without reranking the top-1,000 4-gram sampled repairs using the neural reranker. Since we sample but do not modify the ranks of repairs beyond the top-1000, these remain unchanged. If initially ranked in the top-1k by the 4-gram model, after reranking, the true repair rarely appears beyond 50\textsuperscript{th} in the list.
