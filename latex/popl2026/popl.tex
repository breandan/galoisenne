...
The implementation essentially consists of five stages.
...
\begin{enumerate}
  ...
  \item $\texttt{reg\_dcode}: \text{REG} \rightarrow (\Sigma^* \times \mathbb{N})^{p\gg1}$ -- generates and scores a large number of repairs.
  \item $\texttt{sel\_top\_k}: (\Sigma^* \times \mathbb{N})^{p\gg1} \rightarrow (\Sigma^*)^{k\ll p}$ -- returns a small set of the most probable repairs.
\end{enumerate}

\noindent We will now elucidate the imperative pseudocode for each stage.
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \For{$\langle h, j, i, k \rangle \textbf{ in } [0, n]^2\times[0, d_{\max}]^2$\vspace{1.58cm}}
      \State \vspace{-1.78cm}\[\hspace{1cm}\delta \gets \delta \cup\: \!\left\{
          \begin{alignedat}{9}
            &\;& q_{h,i} &\hspace{-0.1cm}\overset{[\neq\sigma_{j+1}]}{\rightarrow} &q_{j,k} &\qquad& \text{if}\;& h = j   &\:\land\:& i = k-1  &\qquad& \duparrow\\[1pt]
            && q_{h,i}   &\overset{[\neq\sigma_j]}{\rightarrow} &q_{j,k} &&       \text{if}\;& h = j-1 &\:\land\:& i = k-1  &&       \ddiagarrow\\
            && q_{h,i}   &\overset{[=\sigma_j]}{\rightarrow}    &q_{j,k} &&       \text{if}\;& h = j-1 &\:\land\:& i = k    &&       \drightarrow\\
...
Now, the \texttt{cfl\_fixpt} procedure will take an arbitrary acyclic NFA.
...
\State $A \gets \big[\bs \textbf{ if } \exists s \in \Sigma \mid (q \overset{s}{\rightarrow} q') \in \delta \textbf{ else } \ws\big]_{q,q' : Q}$ \Comment{Set up the adjacency matrix.}
\State $R \gets \exp(A)$ \Comment{Solve for reachability matrix.}
...
\For{$\langle p, r, w \rangle \textbf{ in } Q^2\times V$} \Comment{Iterates one step of $M_{i+1} = M_i + M_i^2$.}
  ...
  \State $ M[p, r, w] \gets \bs \textbf{ if } \exists q: Q, x, z: V \mid M[p, q, x] \land M[q, r, z] \land (w \rightarrow x z) \in P \textbf { else } \ws$
...
Note that we can short-circuit in two places: if $M_{i+1} = M_i$, or when two states $q, q'$ are unreachable. Once we obtain $M_\infty$, we can immediately tell whether $\ell_\cap \neq \varnothing$ by checking $M_\infty[q_{0,0}, q', S] = \bs$ for some $q': F$, otherwise if no such $q'$ exists, then it must be empty and we must increase $d_\max$.

Now, we can proceed to build the regular expression representing $\ell_\cap$.
...
Now, we can decode the regular expression.
...
Finally, we can take the highest-probability repairs.
