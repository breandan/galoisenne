...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}[1]
    ...
    \For{$\langle h, j, i, k \rangle \textbf{ in } [0, n]^2\times[0, d_{\max}]^2$\vspace{1.65cm}}
      \State \vspace{-1.85cm}\[\hspace{1cm}\delta \gets \delta \cup\: \!\left\{
          \begin{alignedat}{9}
            &\;& q_{h,i} &\overset{[=\err\sigma_i]}{\rightarrow}    &q_{j,k} &\qquad& \text{if}\;& h = j   &\:\land\:& i = k-1  &\qquad& \duparrow\\[2pt]
            && q_{h,i}   &\overset{[\neq\err\sigma_i]}{\rightarrow} &q_{j,k} &&       \text{if}\;& h = j-1 &\:\land\:& i = k-1  &&       \ddiagarrow\\[2pt]
            && q_{h,i}   &\overset{[\neq\err\sigma_i]}{\rightarrow} &q_{j,k} &&       \text{if}\;& h = j-1 &\:\land\:& i = k    &&       \drightarrow\\[2pt]
            && q_{h,i}   &\overset{[=\err\sigma_i]}{\rightarrow}    &q_{j,k} &&       \text{if}\;& 1 \leq j - h - 1 \leq d_{\max} &\:\land\:& 1 \leq k - i \leq d_{\max}   && \knightarrow\;\\[2pt]
          \end{alignedat}
        \right\}\]
    \EndFor
    \State $I \gets \{q_{0,0}\}, F \gets \{q_{i, j} \mid n - i + j \leq d_{\max}\}$
    \State \Return $\langle Q, \Sigma, \delta, I, F\rangle$  \Comment{Returns a fully constructed Levenshtein automaton.}
  \end{algorithmic}
\end{algorithm}
