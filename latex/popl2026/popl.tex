...
The primary shortcoming with existing approaches is that they generate a very low number of total repairs. Even if the model in question guarantees each repair is grammatically sound or has good statistical generalization properties, it is likely to miss the intended repair when there are a large number of equidistant or equiprobable repairs from which to choose.

To address this problem, the method we propose in this paper explicitly generates and reranks every repair within a fixed edit distance. This ensures that the true repair is retrieved and scored, assuming it is only a few edits apart. Specifically, we adapt the Bar-Hillel construction from formal language theory to construct a regular expression representing the finite language of nearby valid repairs, then enumerate up to $2^{20}$ distinct words from this language, rerank the enumerated set, and present the top dozen or so results to the user, all within a few hundred milliseconds.

To achieve this, we design, develop and benchmark a new algorithm for syntax repair based on matrix exponentiation and derivative-based incremental decoding. This algorithm makes aggressive use of communication-free parallelism, making it readily executable by off-the-shelf GPU and SIMD co-processors. We provide a reference implementation for this algorithm on the WebGPU platform and demonstrate these resources, which typically sit idle during text editing, can be profitably used to deliver real-time program repair in practical usage scenarios.
