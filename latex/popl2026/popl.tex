...
\resizebox{.46\textwidth}{!}{\input{experiments/rank_cdf}}
\vspace{-0.8cm}
\caption{Observed improvement in rank before and after applying the transformer-reranker.}
\label{fig:rank_cdf}
\vspace{-0.3cm}
\end{wrapfigure}

We can quantify the improvement in rank by comparing CDF of the true repair's rank across the test set, with and without reranking the top-1,000 4-gram sampled repairs using the neural reranker. Since we sample but do not modify the ranks of repairs beyond the top-1000, these remain unchanged. If initially ranked in the top-1k results by the 4-gram model, the true repair rarely appears below 50\textsuperscript{th} in the reranked list.
