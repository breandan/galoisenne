...
\vspace{-0.3cm}
\caption{CFL intersection with the local edit region around a broken code snippet, where $d^*=3$ is the langauge edit distance (LED).}
\vspace{-0.3cm}
\end{wrapfigure}
...
We observe that slightly over half of the code snippet pairs in the StackOverflow dataset contain fewer than 80 tokens and four lexical edits, which are computational feasible to process in a few hundred milliseconds. We also note a slight primacy or recency bias in the edit locations, evidenced by a large fraction of human repairs which modify the boundaries of the broken code snippet.
...
For our first experiment, we measure the precision of our repair procedure at various lengths and Levenshtein distances, comparing Tidyparse against Seq2Parse, and BIFI on the same test set.
...
Tidyparse attains SoTA top-1 precision versus Seq2Parse and BIFI across all lengths and edit distances. The Precision@10 of our method even outperforms with BIFI's Precision@20k, whereas our Precision@10 is Pareto-dominant across all lengths and edit distances and precision values. We report the raw data from these experiments in Appendix~\ref{sec:raw_prec_data}.
...
%\vspace{-2cm}
%\begin{center}
%\resizebox{.44\textwidth}{!}{\hspace{-0.8cm}\input{sankey}}
%\end{center}
%\vspace{-0.5cm}
...
The primary question of interest is, to what extent does the neural reranker improve precision relative to a na\"ive decoding strategy? For comparison, we use an 4-gram based repair, sans reranking. That is, we form the language intersection, decode it with a 4-gram model, sort the repairs by their respective 4-gram probabilities, and without further processing evaluate Precision@$10^{\{0, 1, 2, 3\}}$.
...
\caption{4-gram repairs. 4-gram Precision@1000 is an upper bound on Tidyparse Precision@k, since our reranker takes as input the 1000 most probable 4-gram samples.}\label{fig:adaptive}
\end{figure}
...
\resizebox{.45\textwidth}{!}{\input{rank_freq}}
\caption{Observed improvement in rank prior and subsequent to neural reranking (note, y-axis is log-scaled).}
\end{wrapfigure}

We can quantify the improvement in rank by comparing the histogram of the true document's rank across the test set, with and without reranking the top-1,000 4-gram sampled repairs using the neural reranker. Since we sample but do not modify the ranks of repairs beyond the top-1000, these remain unchanged. If initially ranked in the top-1k results by the 4-gram model, the true repair rarely appears below 50\textsuperscript{th} in the reranked list.
...
Finally, we evaluate the impact of increased parallelism on repair throughput. We balance the StackOverflow dataset across edit distances and run DFA sampler for up to 10s, then measure the total wallclock runtime to retrieve the full set of candidate repairs as a function of the number of additional CPU cores assigned, which we exercise to both construct the intersection grammar and sample from it. We compare this with the GPU sampler on the same dataset.
...
In Fig.~\ref{fig:timings}, we plot the end-to-end repair timings by measuring the wallclock runtime of the repair algorithm (\S~\ref{sec:implementation}) across the test-set. This wall-clock runtime corroborates the complexity analysis (\S~\ref{sec:method}) showing a distinct lower bound. While short repairs finish quickly, latency is positively correlated with length and edit distance. Our method is typically able to saturate the admissible set for 1- and 2-edit repairs before timeout, while 4+-edit throughput starts becoming constrained by compute around 10s, when Python's admissible set approaches a volume of $10^5$ valid edits. This bottleneck can be relaxed with a longer timeout, however we anticipate that a much longer delay will begin to tax the patience of most users, and so we consider 10s a reasonable upper bound for repair latency.
