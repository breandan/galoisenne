...
Tidyparse attains SoTA top-1 precision versus Seq2Parse and BIFI across all lengths and edit distances, with a slightly negative correlation between length and precision. Unexpectedly, precision does not monotonically decrease with edit distance, where Tidyparse's double-edit Precision@1 slightly outperforms single-edit Precision@1 across the test set. Although Seq2Parse outperforms BIFI by a weak margin, these results are also mixed for the double-edit repair case. The nonlinear correlation between edit distance and repair precision clearly holds across all three models.
...
For the next experiment, we evaluate the BIFI model, giving it a significant compute and latency advantage with unlimited time to sample 20k repairs and compare against the Precision@10 of our approach with a timeout of 10s. Since Tidyparse does use an LLM for next-token decoding, it can  evaluate and rerank a much larger set of repairs in the allotted time. In the case of Seq2Parse, the authors' default implementation does not support sampling more than a single repair, so we did not compare its Precision@k for higher k. The raw data from these experiments can be found in Appendix~\ref{sec:raw_prec_data}.
...
\noindent We present a Sankey diagram of the Tidyparse repair pipeline in Fig.~\ref{fig:sankey}. Across 2,238 test set repairs filtered by length and distance ($\lfloor|\err\sigma| / 10\rfloor \in [0, 8], \Delta(\err\sigma, \sigma') < 4$), we evaluated Tidyparse with a timeout of 10s and tracked individual repair outcomes. In 607 cases, the human repair was not contained in the langauge intersection and thus never sampled, in 1,631 cases the human repair was sampled, of which 675 cases the first prediction matched the human repair, in 1,242 cases, the human repair was in the top-10 results, and in the remaining 389 cases the human repair was drawn, but ranked lower than tenth in the final results.

%The remaining experiments in this section were conducted on a 16-core ARM64 M4 Max with 128 GB of memory. We balance the StackOverflow dataset across Levenshtein distances, then measure the number of samples required to draw the exact human repair across varying Levenshtein radii. This tells us of how many samples are required on average to saturate the admissible set.
%
%\begin{figure}[h!]
...
The primary question of interest here is to what extent does the neural reranker improve precision relative to a na\"ive decoding strategy? For comparison, we use an 4-gram based repair sans reranking. That is, we decode the langauge intersection with a 4-gram model, sort the repairs by their respective 4-gram probabilities, and without further processing evaluate Precision@$10^{\{0, 1, 2, 3\}}$.
...
We can quantify the ranking improvement by comparing a CDF of the true repair's rank across the test set of human repairs, before and after using the transformer to rerank the top thousand sampled repairs. Since we decode but do not rerank less probable repairs, reranking does not affect repairs originally ranked lower than 1k. Repairs that were initially ranked in the top thousand results by 4-gram probability tend to place between 1\textsuperscript{st} and 10\textsuperscript{th} in about 75\% of repair instances after reranking.
...
Finally, we investigate the impact of increased parallelism on repair throughput. Using the same dataset, we measure the total wallclock runtime to construct the Levenshtein automaton, form the intersection grammar, decode and rank the entire intersection langauge $(\ell_\cap)$. Then, using a GPU implementation of the algorithm described in \S~\ref{sec:implementation} as a drop-in replacement for the CPU version, we plot the individual repair timings across the test set.
\begin{wrapfigure}{l}{0.38\textwidth}
\vspace{-0.2cm}
...
\noindent As shown in Fig.~\ref{fig:timings}, latency depends on various factors but supports the complexity analysis (\S~\ref{sec:method}), exhibiting a clearly superlinear but subexponential runtime profile. While real world performance can vary based on edit distance, intersection volume and other environmental factors, GPU runtime generally has lower variance and confers a noticeable speedup, on average 2-3x for most repair scenarios. Our GPU implementation is typically able to exhaustively decode the intersection for single-edit repairs shorter than 80 tokens before 10s, whereas longer, multi-edit repairs can begin to strain throughput. This timeout can be arbitrarily extended, however we anticipate a much longer delay would begin to tax the patience of most users, and consider it a reasonable upper bound for repair latency.
...
%  We identify three broad categories of limitations in evaluating Tidyparse and suggest directions for future work: naturalness, complexity, and toolchain integeration.
