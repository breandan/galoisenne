...
\noindent Though slightly more verbose, we find the topology induced by the first version of the $\lor$ case to be slightly more favorable for minimization. To minimize, we will use Brzozowski's algorithm~\cite{brzozowski1962canonical} to obtain the unique minimal DFA, $D^*_\cap \equiv_\mathcal{L} G_\cap$. Since $\mathcal{L}(G_\cap)$ is finite, the DFA must be acyclic and thus representable as an upper triangular matrix under a topological ordering of $\delta$.
...
(I-A)^{-1} &= \hspace{1cm}I + A \hspace{1cm}+\hspace{1.05cm} A^2 \hspace{1.07cm}+\hspace{1cm} A^3 \hspace{0.9cm}+\hspace{0.9cm} A^4\\
  ...
  &= \begin{tiny}\begin{pmatrix}
          1   & 1  & 1  & \underline{1} & 2 & \underline{2} \\
              ...
              &    &    &   & 1 & 1 \\
              &    &    &   &   & 1 \\
\end{pmatrix}\end{tiny} \text{ therefor, } \big|\mathcal{L}(D_\cap^*)\big| = C\big(q_0, \{q_3, q_5\}\big) = \underline{1} + \underline{2} = 3.
\end{align}
...
\begin{algorithmic}[1]
  ...
  \For{$\langle h, j, i, k \rangle \textbf{ in } [0, n]^2\times[0, d_{\max}]^2$\vspace{1.34cm}}
    \State \vspace{-1.65cm}\[\hspace{0.5cm}\delta' \gets \!\left\{
        ...
        \end{alignedat}
      \right\phantom{\}}\]
    \State $Q \gets Q \cup \big\{\pi_1(\delta'), \pi_3(\delta')\big\}, \delta \gets \delta \cup \{\delta'\}$
  \EndFor
  \State $I \gets \{q_{0,0}\}, F \gets \{q_{i, j} \mid n - i + j \leq d_{\max}\}$
  \State \Return $\langle Q, \Sigma, \delta = Q \times {\color{orange}(\Sigma \rightarrow \mathbb{B})} \times Q, I, F\rangle$  \Comment{Returns a [nominal] Levenshtein automaton.}
\end{algorithmic}
...
\[
f_\theta(\err\sigma: \bar\ell, \sigma: \ell_\cap): \mathbb{R} = \text{MLP}_{\theta}\big(T_\theta(\err\sigma), T_\theta(\sigma)\big)
\]
