...
Taken together, these results provide strong evidence to support the central claim from the introduction (\S~\ref{sec:intro}): \textit{existing syntax repair methods simply generate far too few repairs to be effective}. By extensively generating and evaluating a large number of repairs within a fixed edit distance, we show it is possible to predict the author's intent far more reliably, with greater precision and lower latency than competing methods which rely solely on transformer-based neural networks.
