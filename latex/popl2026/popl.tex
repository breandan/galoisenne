...
\noindent Though slightly more verbose, we find the topology induced by the first version of the $\lor$ case to be slightly more favorable for minimization. To minimize, we will use Brzozowski's algorithm~\cite{brzozowski1962canonical} to construct the unique minimal DFA, $D^*_\cap \equiv_\mathcal{L} G_\cap$. Since $\mathcal{L}(G_\cap)$ is finite, the DFA must be acyclic and thus representable as an upper triangular matrix under a topological ordering of $\delta$.
...
\begin{align}
  C(A, F): \mathbb{N}^{|Q|\times|Q|} \times 2^Q \rightarrow \mathbb{N} = \sum_{\mathclap{q_\omega \in F}}(I-A)^{-1}[q_\alpha, q_\omega] &= (I + A + A^2 + \ldots + A^{|Q|-1})[q_\alpha, q_\omega]
\end{align}
...
(I-A)^{-1} &= \hspace{1cm}I + A \hspace{1cm}+\hspace{1.05cm} A^2 \hspace{1.07cm}+\hspace{1cm} A^3 \hspace{0.9cm}+\hspace{0.9cm} A^4\\
  ...
  &= \begin{tiny}\begin{pmatrix}
          1   & 1  & 1  & \underline{1} & 2 & \underline{2} \\
              ...
              &    &    &   & 1 & 1 \\
              &    &    &   &   & 1 \\
\end{pmatrix}\end{tiny} \text{ therefor, } \big|\mathcal{L}(D_\cap^*)\big| = C\big(A, \{q_3, q_5\}\big) = \underline{1} + \underline{2} = 3.
\end{align}
