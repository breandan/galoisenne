...
Prior research on syntax repair can be classified into exact and approximate methods. In the former, rule-based methods are used to locate a suitable alternative. While exact, these methods are too weak to model the full distribution of natural source code and must rely on relatively brittle heuristics. In the case of neural program repair, the set of all strings is typically used as a base distribution, which is then learned using a dataset of pairwise errors and fixes. Though statistically more robust, neural language models typically use approximate inference and thus require some form of postprocessing or rejection sampling to ensure the results conform to the grammar.

The primary shortcoming with existing approaches is that they generate a very low number of total repairs. Even if the model in question guarantees each repair is grammatically sound or has good statistical generalization properties across realistic usage scenarios, it is likely to miss the intended repair when there are a large number of approximately equidistant or equiprobable repairs from which to choose.

To address this problem, the method we propose in this paper explicitly generates and reranks every repair within a fixed edit distance. This ensures that the true repair is retrieved and scored, assuming it is only a few edits apart. Specifically, we employ the Bar-Hillel construction from formal language theory to generate a regular expression representing the finite language of nearby valid repairs, then enumerate up to $2^{20}$ distinct words from this language, rerank the enumerated set, and present the top dozen or so results to the user, all within a few hundred milliseconds.

We evaluate our approach on a dataset of human syntax errors and fixes fewer than five lexical edits and shorter than 120 tokens, large enough to fit a few lines of source code in realistic programming languages and editing scenarios. Our work shows this technique is highly effective at predicting the true repair across a dataset of Python source code, on average 5x more accurately than the previous state of the art at comparable latency and compute thresholds.
...
Now, we are ready to consider the general case of syntax repair, in which case the edit locations are not localized but can occur anywhere in the string. In this case, we construct a lattice of all possible edit paths within a fixed distance. This structure is called a Levenshtein automaton.
