...
To count the number of unique repairs, we will first proceed by converting $G_\cap$ to an automaton. Since $\mathcal{L}(G_\cap)$ is finite, it must be representable as an FSA. Recalling the standard definition, we will define an NFA, $\mathcal{N} = \langle Q, \Sigma, \delta, q_\alpha, F \rangle$ which takes the following form:
...
\noindent Though slightly more complex to express, we find the topology induced by the second version of the $\lor$ case is more favorable to minimization. This NFA can be determinized, minimized using Brzozowski's algorithm~\cite{brzozowski1962canonical} and finally analyzed to count the number of unique paths.

Continuing with our running example from Sec.~\ref{sec:repair_ex}, we will have the following NFA and DFA:
...
\begin{figure}[H]
  ...
  \caption{FSA for $\ell_\cap$ before and after determinization.}
\end{figure}

This however requires going through the NFA, which is fairly tedious. Instead, a more direct route is to construct a DFA directly, then minimize it.
