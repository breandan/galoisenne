...
\begin{algorithmic}[1]
  ...
  \State \vspace{-0.42cm}\[\hspace{0.75cm}M'[p, r, w] \gets M[p, r, w] \vee \bigvee_{\substack{q: Q_{pr}\\x, z: V}} \big\{M[p, q, x]\cdot M[q, r, z] \mid (w \rightarrow x z) \in P\big\}\]
  \EndFor
  \State $\textbf{if } M=M' \textbf{ then break else } M \gets M'$
  \EndFor \vspace{0.2cm}
  \State \vspace{-0.42cm}\[\hspace{-7.7cm}\textbf{return }\bigvee_{q': F} M[q_{0, 0}, q', S]\vspace{-0.8cm}\] \Comment{Union regexes for all total parses yielding S.}\vspace{0.4cm}
\end{algorithmic}
\end{algorithm}
