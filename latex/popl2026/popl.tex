...
Recall that every regular language is also context-free a fortiori. So, given an $(\varepsilon, \land)$-free regular expression,~\footnote{Loosely equivalent to a star-free regular expression in the parlance of formal language theory, sans complementation.} we can construct an equivalent CFG with productions $P(e)$ as follows:
...
To count the number of unique repairs, we will first proceed by converting $G_\cap$ to an automaton. Since $\mathcal{L}(G_\cap)$ is finite, it must be representable as an FSA. Recalling the standard definition, we will define an NFA, $\mathcal{N} = \langle Q, \Sigma, \delta, q_\alpha, F \rangle$ which takes the following form: %$Q_{e} = N_Q(e), \delta_{z} = N_\delta(e), $:
\begin{equation}
  \hspace{-0.2cm}N(e) =
  \begin{cases}
    \begin{alignedat}{8}
      ...
      &&\,,\, q_\alpha\,\,, \{q_{\omega}\}
      &&\big\rangle
      &&\quad \text{if } e = \Sigma \\[0.5em]

      &\big\langle Q_{x} \cup Q_{z}
      &&\,,\, \{ q \overset{\sigma}{\rightarrow} q_{\alpha z} \mid q \overset{\sigma}{\rightarrow} q_{\omega}^{\in F_x} \} \cup \delta_{x} \cup \delta_{z}
      &&\,,\, q_{\alpha x}\,, F_{z}
      &&\big\rangle
      &&\quad \text{if } e = x \cdot z \\[0.5em]

      &\big\langle Q_{x} \cup Q_{z} \cup \{q_{\alpha e}\}
      &&\,,\, \{ q_{\alpha e} \overset{\sigma}{\rightarrow} q \mid q_{\alpha\{x, z\}} \overset{\sigma}{\rightarrow} q \}  \cup \delta_{x} \cup \delta_{z}
      &&\,,\, q_{\alpha e}\,, F_{x} \cup F_{z}
      &&\big\rangle
      &&\quad \text{if } e = x \lor z\hspace{-0.5cm}
    \end{alignedat}
  \end{cases}
\end{equation}

\noindent This NFA can be determinized, minimized using Brzozowski's algorithm~\cite{brzozowski1962canonical} and finally analyzed to count the number of unique paths.

For example, suppose we have the syntactically invalid Python string, \texttt{NAME = NAME . NAME ( NUM : , NUM : )}. The GRE recognizing the language intersection can be translated into an equivalent DFA. After minimization, this will take the following form:
...
The candidate repairs contain every nearby edit, filtered by grammatical admissibility. So every repair will be valid, simple and at least somewhat natural, however the list may still be quite long. No reasonable user would be willing to skim through $10^5$ suggestions to find their intended repair, especially since they could have easily written it themselves in a few seconds.

So, we will sort this list by a neural reranker. The ensuing technique falls under the umbrella of the ``learning-to-rank'' problem in machine learning. In their terminology, the broken code snippet is a ``query'' and the list of candidates are ``documents''. If we can guarantee the documents are exhaustive, they will necessarily include the true repair which need only be surfaced.

The model we use is fairly run-of-the-mill and by no means novel. To introduce it, we will need to overload some concepts, so the reader is trusted to contextually interpret $\mathcal{L}$ as denoting a ``loss function'' instead of a langauge, and the derivative operator~\footnote{There is a connection to the Brzozowski derivative, but to refrain from digression we refer the reader to ~\cite{elliott2019generalized} for details.} as the directional rate of change of a differentiable manifold over the parameter space of a neural network. Matrix multiplication remains more or less the same as before, except now over the reals.

We omit the definition of a transformer-encoder in lieu of Strobl et al.'s excellent survey~\cite{strobl2024formal}, except to say that it is a function $T_\theta: \Sigma^* \rightarrow \mathbb{R}^k$. This we will use to embed the query and documents, then we will use a multilayer perceptron (MLP) to return a numerical score for the relevance labels. The full architecture will be something like:

\[
f_\theta(\err\sigma: \Sigma^+, \sigma: \Sigma^+): \mathbb{R} = \text{MLP}_{\theta}\big(T_\theta(\err\sigma), T_\theta(\sigma)\big)
\]
...
We will train it all up via stochastic gradient descent (SGD) with a batch size of 16 for 11,200 steps.
