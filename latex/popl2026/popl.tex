...
Using a symbolic predicate in the construction of the NFA avoids the unnecessary the creation of $2(|\Sigma| - 1)\cdot|\sigma|\cdot d_\max$ arcs and reduces the representation size of the resulting automaton, but does not affect the underlying semantics. Since negation is only permitted over terminals and never propagates further, complementation can be stricken from the usual GRE axioms (Thm.~\ref{thm:gre_def}).
