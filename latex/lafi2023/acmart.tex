...
%  \noindent Depicted above is a SAT tensor representing \hlgray{$\sigma_1\:\sigma_2\:\sigma_3$}$\:\_\:\ldots\:\_$ where shaded regions demarcate known bitvector literals $\mathcal{L}_{r,c}$ (i.e., representing established nonterminal forests) and unshaded regions correspond to bitvector variables $\mathcal{V}_{r,c}$ (i.e., representing seeded nonterminal forests to be grown). Since $\mathcal{L}_{r,c}$ are fixed, we precompute them outside the SAT solver.
  ...
  Given some unparseable string, i.e., $\squiggly{\sigma_1\ldots\sigma_n}: \Sigma^n \cap\mathcal{L}(\mathcal{G})^c$, where should we put holes to obtain $\sigma' \in \mathcal{L}(\mathcal{G})$? We can estimate the effect of perturbing $\sigma$ on $\Lambda_\sigma^*$ by stochastically sampling \textit{minibatches} $\bm{\sigma}\sim\Delta_{k}(\sigma)$ from the Levenshtein k-ball centered on $\sigma$, i.e., the space of all possible Levenshtein edits $\leq k$, and backpropagating $\nabla S$ across upper-triangular entries of $\mathcal{M}^*$. In other words,
