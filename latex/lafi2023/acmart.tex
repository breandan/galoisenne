...
%    \end{pmatrix}
  ...
  A well-known result in formal language theory is that the family of CFLs is not closed under intersection. For example, let us consider $\mathcal{L}^\cap := \mathcal{L}(\mathcal{G}_1) \cap \mathcal{L}(\mathcal{G}_1)$, defined as follows:

  \begin{table}[H]
  \begin{tabular}{llll}
    $\mathcal{G}_1 := \big\{\;S \rightarrow L R$ & $L \rightarrow a b \mid a L b$ & $R \rightarrow c \mid c R\;\big\}$\\
    $\mathcal{G}_2 := \big\{\;S \rightarrow L R$ & $R \rightarrow b c \mid b R c$ & $L \rightarrow a \mid a L\;\big\}$
  \end{tabular}
  \end{table}
  ...
  \begin{figure}[H]
  \includegraphics[width=0.1668\textwidth]{../figures/angle2.png}
  \includegraphics[width=0.147\textwidth]{../figures/angle3.png}
  \includegraphics[width=0.145\textwidth]{../figures/angle4.png}
    \caption{Orientations of a $\bigcap_{i = 1}^4 \mathcal{L}(\mathcal{G}_i) \cap \Sigma ^9$ configuration.}
  \end{figure}
