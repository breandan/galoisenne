...
\begin{proof}[Proof sketch]
  At least one of the following must hold for $w \in V$ to parse some path $p \rightsquigarrow r$ in A:
...
\noindent This suggests a dynamic programming solution. Let M be a matrix of type $RE^{|Q|\times|Q|\times|V|}$  indexed by $Q$. Since we assumed $\delta$ is acyclic, there exists a topological sort of $\delta$ imposing a total order on $Q$ such that $M$ is strictly upper triangular (SUT). We will initialize it as follows:
...
Broadly interested in questions related to formal languages and finite model theory, following the Carnap program of logical syntax. Encoding semantics into syntax would allow us to do type checking and static analysis in the parser. A few lines of attack here:
