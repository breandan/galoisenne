...
\begin{frame}[fragile]{Our Contribution}
  ...
  Given a linear conjunctive language $\ell: \mathcal{L}(\mathcal{G})$ and an invalid string $\err{\sigma}: \Sigma^*$, find every syntactically admissible edit $\tilde{\sigma}$ satisfying $\{\tilde{\sigma} \in \ell \mid \Delta(\err{\sigma}, \ell) < r\}$, ranked by a metric $\Delta$, under hard realtime constraints.
  ...
  \textit{Finds syntactically valid strings within a small edit distance, ranked by tokenwise similarity and statistical likliehood.}
\end{frame}


\begin{frame}[fragile]{An Simple Existience Proof}
  \begin{lemma}
  ...
  \begin{proof}
  Since $\ell$ is nonempty, it must have at least one inhabitant $\sigma \in \ell$. Let $\tilde{\sigma}$ be the smallest such member. Since $\tilde{\sigma}$ is a valid string, by definition it must be that $|\tilde{\sigma}|<\infty$. Since we know $\err{\sigma} \notin \ell$, it follows that $0 < \Delta(\err{\sigma}, \ell)$. We must now consider two cases, either $\tilde{\sigma} = \varepsilon$, or $0 < |\tilde{\sigma}|$.

  \begin{itemize}
    \item If $\tilde{\sigma} = \varepsilon$, then $\Delta(\err{\sigma}, \tilde{\sigma}) = n$ by full deletion on $\err{\sigma}$, or
    \item If $0 < m$, then we have $\Delta(\err{\sigma}, \tilde{\sigma}) \in (0, \max(m, n)]$.
  \end{itemize}

  In either case, it follows $\Delta(\err{\sigma}, \ell) \leq \max(m, n)$ and $\ell$ is reachable via a finite nonempty set of Levenshtein edits, i.e., $0 < \Delta(\err{\sigma}, \ell) < \infty$.
    \end{proof}
\end{frame}
