...
%\begin{figure}[h!]
    ...
    \node [below=0.7cm of pro2b, xshift=-0.25cm] {\Large\textbf{Language intersection}};
    ...
    \node [above=0.07cm of const, xshift=1.5cm] {(Algorithm A)};
    ...
    \node [above=0.07cm of grwa, xshift=1.5cm] {(Algorithm B)};
    \node (rank) [process, below of=grwa, yshift=-0.5cm] {Convert to DFA and walk};
    \node [above=0.1cm of rank, xshift=1.5cm] {(Algorithm C)};
    %          \node (vlmc) [io2, right of=rank, xshift=3cm] {Markov chain};
    \node [below=0.3cm of rank, xshift=2.5cm] {\Large\textbf{Repair decoding}};
...
\State $i \gets \texttt{RandIntWoR}(\texttt{seed++}, |T|)}$ \Comment{Sample $\mathbb{Z}_{|T|}$ uniformly WoR.}
\State $\hat{a} \gets \hat{a} + \varphi(T, i)$ \Comment{Decode fresh tree and add.}
%\Until{interrupted or $\texttt{seed} = |T|$.}
\EndFor
\State \Return $\hat{a} \textbf{ ranked by } NLL\big(\cdot \Rightarrow \Sigma^* \big)$
\end{algorithmic}
\end{algorithm}
