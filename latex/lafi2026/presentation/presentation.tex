...
\begin{frame}{Finite intersection as matrix exponentiation on $(2^V,\oplus,\otimes)$}
  ...
  \vspace{-0.35cm}
  \begin{align*}
    \exp(M_0)&=\sum_{i=0}^{\infty} M_0^i \;=\;\sum_{i=0}^{|Q|} M_0^i \quad\text{($\alpha_\varnothing \Leftrightarrow $ S.U.T. $\Rightarrow$ nilpotent).}\\
...
\begin{frame}{Future work}
  \begin{itemize}
    ...
    \item Laziness: instantiate CFG productions during parsing
    \item Extend to richer type systems, e.g., polymorphism, higher-order functions, subtyping, nested scope
    \item \href{https://arxiv.org/pdf/2512.01036v1}{``A Relevance Sampler for $\mu$Rust$_{\text{sl}}$''} (Considine, 2025) explores a straight-line fragment of Rust with nominal relevance
    \item \href{https://arxiv.org/pdf/2408.01849}{``
A Tree Sampler for Bounded CFLs''} (Considine, 2024) describes a uniform sampler for finite CFL intersections
    \item Other FPT embeddings. Open to suggestions!
    \item Applications to program synthesis and repair
    \item Try it yourself at: \href{https://tidyparse.github.io}{https://tidyparse.github.io}
  \end{itemize}
\end{frame}

\begin{frame}{Acknowledgements}
  \centering\Large{Thank you!}\vspace{0.5cm}
  \begin{columns}
  \begin{column}{0.5\textwidth}
  \begin{itemize}
    \item Ori Roth
