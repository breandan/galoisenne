...
% =========================================================
    ...
    \null\hspace*{2.8cm}\begin{minipage}[c]{0.86\columnwidth}
        \textbf{Type universe.} We assume a finite universe $\T$ with at two base types $\mathbb{B},\mathbb{N}$, and an ambient global context
...
\vspace{0.8cm}
    ...
    \null\hspace*{2.8cm}\begin{minipage}[c]{0.86\columnwidth}
        where the infix operator typing function $\delta_{\texttt{OPR}}$ is defined as follows:
...
% =========================================================
    ...
    \null\hspace*{2.8cm}\begin{minipage}[c]{0.86\columnwidth}
        Finite slices of a CFL are finite and therefore regular. We use star free regular expressions as a compact algebra for propagating regular constraints during parsing and decoding.
        ...
        where $\varepsilon$ is the empty symbol. We interpret these expressions as denoting regular languages, where $\mathcal{L}(x)\circ\mathcal{L}(z) \coloneqq \big\{a \cdot b \mid a \in \mathcal{L}(x) \land b \in \mathcal{L}(z) \big\}$:
    \end{minipage}
    ...
    \begin{multicols}{2}
      ...
      \begin{eqnarray*}
        \hspace{-0.5cm}\mathcal{L}(&\hspace{-0.35cm} x\cdot z \hspace{-0.35cm}&) = \mathcal{L}(x) \circ \mathcal{L}(z)\text{\footnotemark}\\
    ...
    \vspace{-0.5cm}
    \null\hspace*{2.8cm}\begin{minipage}[c]{0.86\columnwidth}
        Now, for any nonempty $(\varepsilon, \land)$-free regex, \(e\), to witness $\sigma \in \mathcal{L}(e)$:
    \end{minipage}
...
% =========================================================
    ...
    \null\hspace*{2.8cm}\begin{minipage}[c]{0.86\columnwidth}
        \begin{itemize}[leftmargin=1.5cm, labelsep=0.6em]
          ...
          \item Intersection with an acyclic FSA yields a regular expression
          \item Brzozowski derivatives enable incremental autoregressive decoding
          \item Future work: lazy CNF materialization, quotienting (e.g., $\alpha$-equivalence), richer typing (subtyping, polymorphism, substructural constraints).
        \end{itemize}
    \end{minipage}
