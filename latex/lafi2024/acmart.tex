...
\begin{document}
  ...
  \noindent If we define $\hat\sigma_r = \{w \mid (w \rightarrow \sigma_r) \in P\}$, then initialize $M^0_{r+1=c}(\mathcal{G}', e) = \;\hat\sigma_r$ then solve for $M = M + M^2$, the fixedpoint $M_\infty$ is fully determined by the superdiagonal entries:\vspace{-10pt}
