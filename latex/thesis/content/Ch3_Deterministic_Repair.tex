...
A \textbf{Directed Graph (DG)} is a pair $\mathbf{G} = \langle V, E \subseteq V \times V\rangle$ whose structure can be described via its adjacency matrix $A[u, v] = \mathds{1}_{u\rightarrow v \in E}$. Powers of the adjacency matrix ($A^k$) represent the number of directed paths of a given length between any two nodes, i.e., $(A^k)[u,v] = |\{ p: V^k \mid p=u \rightarrow^* v\}|$. For a \textbf{Directed Acyclic Graph (DAG)}, $A$ satisfies the following properties:
...
Effectively, this matrix exponential is what we are computing when we are parsing. We will now generalize it to parsing on an acyclic graph.
...
Let $A$ be an acyclic nondeterministic finite automaton with an unlabeled transition graph $T = \langle Q, \{ u \rightarrow v \mid  s \in \Sigma \land (u \overset{s}{\rightarrow} v) \in \delta\}\rangle$, and let $\prec_T$ be a topological sort on $T$. In other words, $u \prec v \Longleftrightarrow \exists \sigma \in \mathcal{L}(u \Longrightarrow v)$.

We will define a matrix algebra over $2^V$ that decides intersection nonemptiness (INE) of a CFG and FSA. Let $G'=\langle \Sigma, V, P, S\rangle$ be a CFG in CNF and $N: \langle \Sigma, Q, \delta, I, F\rangle$ be an acyclic FSA. We will define , and $\oplus = \cup$ and $\otimes = \ldots$.

\begin{equation}
M_0[u, v] = \{w : V \mid (u \overset{s}{\rightarrow} v) \in \delta \land (w \rightarrow s) \in P \}
\end{equation}

$\odot: Q \times Q \rightarrow 2^Q$ is a function returning the set of midpoints on any path between two states in the FSA.

\[
  p\odot r = \{q \in Q \mid p \rightarrow^* q \rightarrow^* r \}
\]
