...
\noindent directly corresponds standard CYK parsing algorithm presented in \S~\ref{sec:matrix_parsing}. Effectively for matrix semirings, this can be computed using the equivalent construction:

\begin{proof}
  To compute \(M^n\), we use:
  \[
    M^n \;=\;
    \begin{cases}
      M, & \text{if } n=1,\\[3pt]
      \bigl(M^{n/2}\bigr)^2, & \text{if \(n\) is even},\\[3pt]
      \bigl(M^{(n-1)/2}\bigr)^2 \; M, & \text{if \(n\) is odd}.
    \end{cases}
  \]
  Each level of recursion requires at most two matrix multiplications in parallel, yielding \(\mathcal{O}(\log n)\) depth.

  For the sum \(S(n) = M + M^2 + \cdots + M^n\), observe that
  \[
    S(2k) \;=\; S(k) \;+\; M^k \, S(k).
  \]
  which requires $\mathcal{O}(\log_2 n)$ parallel matrix multiplication steps, $M_{i+1} = M_i + M_i^2$.
\end{proof}

We will now generalize this procedure to parsing an acyclic finite state automaton.
