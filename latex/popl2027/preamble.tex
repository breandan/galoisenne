...
\def\flann@rawwave{{\color{red}\kern-1.5pt\scalebox{1.25}[1.5]{\contour{red}{\tenly \char58}}\kern1.5pt}}
\def\flann@mathwave{{\color{red}\contourlength{0.18pt}\scalebox{0.36}[0.56]{\contour{red}{\tenly \char58}}}}
\def\flann@squigglemark{\lower8\p@\hbox{\flann@rawwave}}
\def\squigglyred{%
  \bgroup
  \markoverwith{\flann@squigglemark}%
  \ULon
}
\def\err@math#1#2{%
  \mathord{\hbox{%
    ...
    \copy0
    \kern-\wd0
    \smash{\lower3.5\p@\hbox to \wd0{\hss\flann@mathwave\flann@mathwave\hss}}%
  }}%
}
\newcommand{\err}[1]{%
  \ifmmode
    \mathchoice{\err@math\displaystyle{#1}}{\err@math\textstyle{#1}}{\err@math\scriptstyle{#1}}{\err@math\scriptscriptstyle{#1}}%
  \else
    \smash{\squigglyred{#1}}%
  \fi
}
...
\newcommand{\ddiagarrow}{
  \tikz{
    ...
    \fill (0pt,6pt) circle [radius = 1pt];
    \fill (6pt,6pt) circle [radius = 1pt];
    \draw [orange,-to] (0pt,0pt) -- (6pt,6pt);
  }
}
...
\makeatletter
\newcommand{\leftharpoonrightarrow}{%
  \mathrel{\mathpalette\leftharpoonrightarrow@{}}%
}
\newcommand{\leftharpoonrightarrow@}[2]{%
  \sbox\z@{$\m@th#1\leftrightarrow$}%
  \hbox to \wd\z@{%
    \rlap{$\m@th#1\mkern-2mu\leftharpoonup$}%
    \hfil
    $\m@th#1\mkern 2mu\rightarrow$%
  }%
}
\makeatother
