...
For our final repair procedure, we use the CNF Python grammar, $G_{\text{Python}}$ and let $d_{\max}$ be the smallest value such that $\ell_\cap = \mathcal{L}(G)\cap\mathcal{L}\big(L(\err\sigma, d_{\max} - 1)\big)$ is nonempty. We decode $\ell_\cap$ with the same pretrained $4$-gram model used in reranker training, and pass the top-$512$ results by WDFA probability to the transformer encoder, then finally rerank the top-$512$ by softmax probability and measure the Precision@k across repairs of differing length and edit distance in the test set.
...
\hangafter=-10
\noindent Finally, we investigate the impact of increased parallelism on repair throughput. To simulate a realistic editing scenario, we measure the end-to-end wallclock runtime required to construct the Levenshtein automaton, form the intersection regex $(S_\cap)$, decode and rank the entire intersection language $(\ell_\cap)$. Then, using our GPU implementation of the same algorithm (described in \S~\ref{sec:implementation}), we compare the individual repair timings across the same test set. As shown in Fig.~\ref{fig:timings}, GPU execution has slightly lower variance and confers a 3-5x speedup across common repairs, while the observed end-to-end latency reflects the combined cost of automaton construction, matrix closure, exhaustive decoding, and ranking. The GPU implementation finishes processing nearly all repairs the 10s timeout and about $90\%$ of all instances in under 1s; the CPU implementation may miss that target on longer or multi-edit repairs. We therefore use 10s as a practical upper bound for interactive repair latency.
\par\endgroup
\vspace{0.05cm}
