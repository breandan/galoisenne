...
\begin{theorem}[Witness construction]\label{thm:witness_construction}
  ...
  saturated chart can be lifted to a finite star-free regular expression
  \(S_\cap\) with \(\mathcal{L}(S_\cap)=\mathcal{L}(G)\cap\mathcal{L}(A)\). Thus, \(\Psi(G,A)\) implies a concrete repair witness exists.
\end{theorem}

\begin{proof}
  Let \(E_+\) be \((\varepsilon,\land, \text{star})\)-free REs plus \(\varnothing\), \(e: E_+ ::= \varnothing\mid a\mid e\vee e\mid e\land e\mid e\cdot e\). We will define a provenance semiring variant of Thm.~\ref{thm:parallel_decision_complexity}, using $E_+$ union and concatenation:
  \[
    \begin{aligned}
  ...
  order, the shared representation is an acyclic SFRE DAG. If nonempty, pruning
...
\begin{definition}[Brzozowski derivative]\label{def:brzozowski_derivative}
  Following Brzozowski~\cite{brzozowski1964derivatives}, let \(E\) be the star-free RE,
  \(e: E ::= \varnothing\mid\varepsilon\mid a\mid e\vee e\mid e\land e\mid e\cdot e\).
...
\begin{lemma}[Generation from $E_+$]\label{lem:generation}
  Let \(e ::= a\mid e\cdot e\mid e\vee e\) be productive, i.e., every
