...
\begin{proof}
  ...
  terminals prove the base case, and each step chooses a rule \(w\rightarrow xz\) and midpoint \(q\). Acyclicity bounds accepting paths, hence the bounded closure reaches \(R_\star\).  Taking \(S_\cap=\bigvee_{q_\omega\in F}R_\star[q_\alpha,q_\omega,S]\), (*) gives the claimed language equality. Since products use strict subpaths in topological order, the shared representation is an acyclic RE DAG. If nonempty, pruning \(\varnothing\) yields a productive $E_+$, to which Lemma~\ref{lem:generation} applies.
\end{proof}
