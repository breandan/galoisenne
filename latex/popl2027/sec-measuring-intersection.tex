...
\begin{theorem}[Enumeration]\label{thm:enumeration}
  ...
  \begin{equation}
    ...
    \end{cases}
  \end{equation}
  To enumerate, we construct a bijection between trees and integers, then invoke $\bigcup_{i = 0}^{|e|-1}\big\{\texttt{enum}(e, i)\big\}$:
  \begin{equation}
...
If the FSA, $A_\varnothing$, is ambiguous, then the intersection grammar, $G_\cap$, can be ambiguous.
\end{lemma}

\begin{proof}
Let $\ell$ be the language defined by $G=\{S\rightarrow LR, L \rightarrow\texttt{(}, R \rightarrow\texttt{)}\}$, where $A_\varnothing=L(\err\sigma, 2)$, the broken string $\err\sigma$ is $\texttt{)(}$, and $\mathcal{L}(G_\cap) = \ell \cap \mathcal{L}(A_\varnothing)$. Then $\mathcal{L}(G_\cap)$ contains a word that can be parsed with two different trees: \texttt{\hlred{)}(\hlgreen{)}}  $\Rightarrow S_\cap \rightarrow q_{00}Lq_{21}\phantom{.}q_{21}Rq_{22}$, and \texttt{\hlorange{(}\hlorange{)}} $\Rightarrow S_\cap \rightarrow q_{00}Lq_{11}\phantom{.}q_{11}Rq_{22}$.
\end{proof}
