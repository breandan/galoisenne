...
%      \addlegendentry{3}
  ...
  For the stability profile, we compute the average ratio of unmodified edit locations across repairs of a certain Levenshtein distance, normalized by the total number of tokens. For example, the number of stable locations for a 1-edit repair of length $\~5-10$ tokens is on average $\sim 70\%$ of the total string, whereas for a 2-edit repair of the same length, almost any location can be changed with the right edits. For a fixed edit distance, we observe a decrease in the degrees of caret freedom, which intuitively makes sense, as the locations available to edit by a repair become more heavily constrained by the surrounding context as the string length increases.
