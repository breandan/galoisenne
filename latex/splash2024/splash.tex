...
%        \node (ptree) [io, above of=node2, yshift=-3cm] {$\mathbb{T}_2$};
        \node (node3) [plain, right of=node1] {\textbf{k-best decoding}};
        \node (ngram) [io2, above of=node3, yshift=-3cm] {PCFG};
...
%  \end{enumerate}
  ...
  To decode the repairs, we use a technique called cube pruning, which has been shown to be effective for decoding likely sentences from PCFGs~\cite{huang2005better}. If the language is sufficiently small, it will generate every possible repair, otherwise, it will draw a representative subset containing with high probability the top-k most likely repairs under the PCFG.
...
\end{equation}
  ...
  The previous technique will enumerate parse trees in a given $\mathbb{T}_2$, but in an arbitrary order. While sufficient for the purpose of decoding small finite languages, it can be improved for large finite languages by decoding repairs in order of likelihood. To decode the top-k maximum likelihood results without extracting all repairs and reranking, one can instead define an automata algebra over $\mathcal{M}^{|V|}$, propagating a vector of NFAs, $A$ indexed by the nonterminal $v: V$:
...
%    \end{pmatrix}
 ...
 For every nonterminal in the intersection grammar $qvq' \in V_\cap$, we have $v \in V$. Since we do not use a weighted NFA, we may ignore the states $q, q'$ and treat every intersection nonterminal as the original nonterminal from the PCFG. By avoiding the NFA states, this simplifies the probability table. Let's consider a few illustrative examples using . We use upper case to denote a generic nonterminal, and lower case to denote its concrete instantiation:
  ...
  \begin{table}[h!]
    ...
    \begin{minipage}{3cm}\resizebox{.6\textwidth}{!}{\begin{tikzpicture}
        ...
        \edge {z} {x,y}
      \end{tikzpicture}}\end{minipage}\begin{minipage}{5cm}\begin{align*} P(W\rightarrow XZ) &= P\big((pwr) \rightarrow (pxq)(qzr)\big) \\ &= P(X=x, Z=z \mid W=w)\\ &= \frac{P(W=w, X=x, Z=z)}{\sum_{x', z'}P(X=x', Z=z' \mid W)} \end{align*}\end{minipage}
  \end{table}

Likewise, we can perform this same computation with higher-order dependencies, where $W$ stands for a generic grandparent, and $X, Z$ being the generic parent and pibling:

  \begin{table}[h!]
    \begin{minipage}{4cm}\resizebox{.6\textwidth}{!}{\begin{tikzpicture}
       \node[obs] (z) {$W$};%
       \node[obs,below=of z,xshift=-1cm,fill] (x) {$X$}; %
       \node[obs,below=of z,xshift=1cm] (y) {$Z$}; %
       \node[latent,below=of x,xshift=-1cm] (r) {$R$}; %
       \node[latent,below=of x,xshift=1cm] (t) {$T$}; %
       \edge {z} {x,y}
       \edge {x} {r,t}
    \end{tikzpicture}}\end{minipage}\begin{minipage}{5cm}\begin{align*} P(X\rightarrow RT \mid W\rightarrow XZ) &= P(R=r, T=t \mid W, X, Z)\\ &= \frac{P(W=w, X=x, Z=z)}{\sum_{r', t'}P(R=r', T=t' \mid W, X, Z)} \end{align*}\end{minipage}\end{table}
  \begin{table}[h!]
    \begin{minipage}{4cm}\resizebox{.6\textwidth}{!}{\begin{tikzpicture}
                           \node[obs] (z) {$W$};%
                           \node[obs,below=of z,xshift=-1cm,fill] (x) {$X$}; %
                           \node[obs,below=of z,xshift=1cm] (y) {$Z$}; %
                           \node[latent,below=of x,xshift=1cm] (u) {$U$}; %
                           \node[latent,below=of x,xshift=3cm] (v) {$V$}; %
                           \edge {z} {x,y}
                           \edge {y} {u,v}
    \end{tikzpicture}}\end{minipage}\begin{minipage}{5cm}\begin{align*} P(Z\rightarrow UV \mid W\rightarrow XZ) &= P(U=u, V=v \mid W, X, Z)\\ &= \frac{P(W=w, X=x, Z=z)}{\sum_{r', t'}P(U=u', V=v' \mid W, X, Z)} \end{align*}\end{minipage}\\
  \end{table}
