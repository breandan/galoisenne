...
%  \email{six@utoronto.ca}}
  ...
  \begin{enumerate}
    ...
    \item To extract the repairs, we sample trees without replacement by constructing a bijection between syntax trees and integers, sample integers uniformly without replacement from a finite range, then decode them as trees. (\S~\ref{sec:ptree})
    ...
    \begin{center}
      \resizebox{0.35\textwidth}{!}{
        \begin{tikzpicture}[node distance=2cm]
          ...
          \node [above=0.1cm of pro1] {(\S~\ref{sec:lev_bh},~\ref{sec:matrix_completion})};
          \node (pcfg) [io, left of=pro1, xshift=-3cm] {[P]CFG};
          \node [below=0.1cm of pcfg] {(\S~\ref{sec:prelim})};
          \node (lnfa) [io, right of=pro1, xshift=3cm] {L-NFA};
          \node [above=0.1cm of lnfa, xshift=0.7cm] {(\S~\ref{sec:lev_bh})};
          ...
          \node [above=0.1cm of samp1] {(\S~\ref{sec:ptree})};
          \node (samp2) [process, right of=dec2, xshift=3cm] {Sample $\sigma' \sim P(G_\cap)$};
          \node [above=0.1cm of samp2] {(\S~\ref{sec:ptree})};

          \node (rank) [process, below of=dec2, yshift=-2.5cm] {Rerank by $L_\theta(\sigma')$};
          \node (vlmc) [io, above of=rank, yshift=-0.1cm] {Markov Chain};
          \node [below=0.2cm of vlmc, xshift=0.7cm] {(\S~\ref{sec:ranking})};
...
%          \draw [arrow] (dec2) -- node[anchor=east] {1} (stop);
  ...
  \begin{enumerate}
    ...
    \item $\mathcal{L}(G_\cap)$ is small, in which case we enumerate all possible repairs. Complete enumeration is tractable for the majority of syntax repairs.
    \item $\mathcal{L}(G_\cap)$ is too large to enumerate, so we sample from the intersection grammar $G_\cap$. Sampling is necessary for a minority of remaining cases.
  \end{enumerate}

  As long as we have done our job correctly, the intersection language should contain nearly every repair within a certain Levenshtein distance, and no invalid repairs. This procedure is depicted in Fig.~\ref{fig:flowchart}. In the following section, we will describe how we construct the intersecection grammar (\S~\ref{sec:lev_nfa},~\ref{sec:lev_bh}), then, provide an explicit technique for extracting all repairs contained within its language (\S~\ref{sec:ptree}). Finally, we use an n-gram model to rank and return the top-k results by likelihood (\S~\ref{sec:ranking}).
...
%  If so, we require the sampler to have (3) parallel decomposability and generate samples (4) uniformly, both (5) with and (6) without replacement in (7) constant time and space. These requirements motivate the need for a specialized generating function.
  ...
  \noindent The northeasternmost entry $\Lambda^*_\sigma$ gives us the recognizer $R(G', \sigma) = [S \in \Lambda^*_\sigma] \Leftrightarrow [\sigma \in \mathcal{L}(G)]$~\footnote{Hereinafter, we use Iverson brackets to denote the indicator function of a predicate with free variables, i.e., $[P] \Leftrightarrow \mathds{1}(P)$.}.
...
If the language is suffiently small, instead of sampling trees, we can sample integers uniformly without replacement from $\mathbb{Z}_{|T|}$ then decode them into trees using $\varphi$. This procedure is the basis for our sampling algorithm and the method we use to decode repairs from the intersection grammar.
  ...
  We compare Tidyparse against two separate baselines, Seq2Parse (2022) and Break-It-Fix-It (BIFI)~\cite{yasunaga2021break} and two datasets. The first dataset~\cite{wong2019syntax} consists of 10k naturally-occurring pairs of Python errors and their corresponding human fixes from StackOverflow, and the second consists of synthetically generated errors in natural Python snippets~\cite{yasunaga2021break}. We compare precision at recovering the ground truth repair across varying edit distances, snippet lengths and latency cutoffs.
...
%    \end{tikzpicture}
  ...
  Next, we measure the precision of our repair procedure at various lengths and Levenshtein distances. We sample the StackOverflow dataset balancing sample sizes across each length interval and edit distance and compare Precision@1 of our method against Seq2Parse and BIFI.
  ...
  \begin{figure}[h!]
    ...
    \caption{Tidyparse, Seq2Parse and BIFI top-1 repair precision at various lengths and Levenshtein distances.}\label{fig:len_dist_prec}
  \end{figure}

  The precision of our method is highly competitive with Seq2Parse and BIFI across all lengths and edit distances, and attains a significant advantage in the few-edit regime. We also measure the precision of our method at various latency cutoffs, and compare it against Seq2Parse and BIFI.
...
%  While adaptive sampling is able to saturate the admissible set for 1- and 2-edit repairs before the timeout elapses, 3-edit throughput is heavily constrained by compute around 16 lexical tokens, when Python's Levenshtein ball has a volume of roughly $6\times 10^8$ edits. This bottleneck can be relaxed with a longer timeout or additional CPU cores. Despite the high computational cost of sampling multi-edit repairs, our Precision@All remains competitive with the Seq2Parse neurosymbolic baseline at the same latency. We provide some qualitative examples of repairs in Table~\ref{sec:appendix}.
  ...
  \subsection{Synthetic repair evaluation}\label{sec:latency}
  ...
  We also compare the completeness of Seq2Parse versus BIFI using the Precision@All metric.
  ...
  Our primary insight leading to state-of-the-art precision is that repairs are typically concentrated near the center of a small Levenshtein ball, and by enumerating or sampling it carefully, then reranking all repairs found we can achieve a significant improvement. We note this heuristic may not apply all grammars, and it may be possible to construct less natural counterexamples where this heuristic fails, although we consider these unlikely in practice.
