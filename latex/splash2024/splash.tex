...
%
  ...
  %
  \begin{abstract}
    We introduce a new technique for correcting syntax errors in arbitrary context-free languages. Our work comes from the observation that syntax errors with a small repair typically have very few unique small repairs, which can usually be enumerated up to a small edit distance then quickly reranked. We place a heavy emphasis on precision: the enumerated set must contain every possible repair within a few edits and no invalid repairs. To do so, we model error correction as a language intersection problem between a Levenshtein automaton and a context-free grammar. To extract the repairs, we sample trees from the intersection grammar, yielding all valid repairs within a certain Levenshtein distance. Finally, we rank those repairs by n-gram perplexity.
...
%
  ...
  As the admissible set $A$ is typically under-constrained, we want a procedure which prioritizes natural and valid repairs over unnatural but valid repairs:
...
%          \draw [arrow] (dec2) -- node[anchor=east] {1} (stop);
  ...
  Each arc plays a specific role. $\duparrow$ handles insertions, $\ddiagarrow$ handles substitutions, $\duparrow$ handles insertions and $\knightarrow$ handles deletions of one or more terminals. Let us consider some illustrative cases.
...
%\begin{prooftree}
  ...
  Next, we will define a measure on Parikh intervals, which will represent the minimum total edits required to transform a string in one Parikh interval to a string in another, across all string pairs.
  ...
  We define the modified Bar-Hillel construction for nominal Levenshtein automata as:
...
\end{prooftree}
  ...
  Finally, once $G_\cap$ is constructed, we then renormalize it by removing all unreachable and non-generating productions following~\cite{firsov2015certified} to obtain $G_\cap^*$, which is usually several orders of magnitude smaller. When this grammar is sufficiently small, it is possible to extract all repairs from it, otherwise we sample it to obtain a subset of admissible repairs.
...
If the language is suffiently small, instead of sampling trees, we can sample integers uniformly without replacement from $\mathbb{Z}_{|T|}$ then decode them into trees using $\varphi$. This procedure is the basis for our sampling algorithm and the method we use to decode repairs from the intersection grammar.
  ...
  Returning to the ranked repair problem (Def.~\ref{def:ranked-repair}), the above procedure returns a set of strings, and we need an ordering over them. We note that any metric is sufficient, such as the perplexity of the string under a large language model, or the probability of the string under a PCFG. We turn to simplest possible solution: the negative log likelihood of the string computed by a variable-order Markov chain. This is a simple, fast, and effective metric for ranking strings, and as we will show, already yields competitive results in practice.

  Specifically, given a string $\sigma: \Sigma^*$, we factorize the probability $P(\sigma)$ as a product of conditional probabilities $P(\sigma_i \mid \sigma_{i-1}\ldots\sigma_{i-n})$ for some $n \in \mathbb{N}$. To compute the score, we use the length-normalized negative log likelihood:

  \begin{equation}
    \text{NLL}(\sigma) \coloneqq -\frac{1}{|\sigma|}\sum_{i=1}^{|\sigma|}\log P(\sigma_i \mid \sigma_{i-1}\ldots\sigma_{i-n})
  \end{equation}

  Then, for each $\sigma \in A(\err\sigma)$, we compute $\text{NLL}(\sigma)$ and return the set of strings in ascending order of their scores. That is, we want a ranked list of strings in $A(\err\sigma)$, where the first string is the most likely repair, and the last string is the least likely repair. To evaluate the quality of our ranked repairs, we primarily use P@k, which is the frequency of repairs in the top $k$ ranked repairs that are lexically equivalent to the human repair.

  Specifically, given a repair model, $R: \Sigma^* \rightarrow 2^{\Sigma^*}$, and a set of test cases $\mathcal{D}_{\text{test}}$, we define the Precision@k as:

  \begin{equation}
    \text{P@k} = \frac{1}{|\mathcal{D}_{\text{test}}|}\sum_{\langle\sigma^\dagger, \sigma'\rangle \in \mathcal{D}_{\text{test}}} 1\big[\sigma' \in \argmax_{\bm{\sigma} \subset R(\sigma^\dagger), |\bm{\sigma}| = k}\sum_{\sigma \in \bm{\sigma}}\text{NLL}(\sigma)\big]
  \end{equation}
