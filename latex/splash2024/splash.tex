...
%    We want a procedure $\hat{R}$, minimizing $\mathbb{E}_{G, \sigma}\big[D_{\text{KL}}(\hat{R} \parallel R)\big]$ and wallclock runtime.
  ...
  To satisfy Def.~\ref{def:maximal-retieval}, we will construct a bijection from syntax trees to integers, sample integers uniformly without replacement, then decode them as trees. Assuming the grammar is unambiguous, each tree will correspond to a unique string in the language. As long as $|\ell_\cap|$ is sufficiently small and we draw enough samples, $\bm\varphi$ is sure to include the most natural repairs and terminate quickly. Then, the problem of ranked repair reduces to sorting retrieved samples by likelihood, which can be done with a low-order Markov model or any suitable scoring function of the implementer's choice.
