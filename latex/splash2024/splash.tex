...
%  \email{six@utoronto.ca}}
  ...
  Syntax errors are a familiar nuisance for programmers, arising due to a variety of factors, from inexperience, typographic mistakes, to cognitive load. Often the mistake is simple to fix, but manual correction temporarily disrupts one's concentration, a developer's most precious and fickle resource. Syntax repair attempts to automate the correction process by modifying a syntactically invalid program so that it conforms to the grammar, saving time and attention.

  Most prior work on syntax repair falls into two high-level categories: (1) \textit{formal methods}, which use handcrafted rules to fix common syntax errors (e.g.,~\cite{aho1972minimum}), and (2) \textit{machine learning}, which use neural language models to repair code (e.g.~\cite{yasunaga2021break, sakkas2022seq2parse}). The former can be effective but typically produces a single repair, while the latter generates more natural edits, but is costly to train, prone to misgeneralization, and difficult to incorporate new constraints thereafter.
...
%  While adaptive sampling is able to saturate the admissible set for 1- and 2-edit repairs before the timeout elapses, 3-edit throughput is heavily constrained by compute around 16 lexical tokens, when Python's Levenshtein ball has a volume of roughly $6\times 10^8$ edits. This bottleneck can be relaxed with a longer timeout or additional CPU cores. Despite the high computational cost of sampling multi-edit repairs, our Precision@All remains competitive with the Seq2Parse neurosymbolic baseline at the same latency. We provide some qualitative examples of repairs in Table~\ref{sec:appendix}.
  ...
  We also measure the relative improvement in throughput (measured by the number of distinct repairs found after a fixed timeout) as a function of the number of additional CPU cores, averaged across 1000 trials. We observe the relative increases logarithmically with the number of additional CPU cores and at least four CPU cores are needed to offset the overhead of synchronization. We recommend using as many CPU cores as possible to maximize latency and throughput. Further speedups may be possible to realize by rewriting the sampler in CUDA, which we leave for future work.
