...
%
  ...
  Syntax repair is the problem of modifying an invalid sentence so it conforms to some grammar. Prior work has been devoted to fixing syntax errors using a variety of handcrafted heuristics including rule-based systems and statistical language models. However, these techniques are often brittle, and are susceptible to misgeneralization. In a prior paper~\cite{sakkas2022seq2parse} published in SPLASH, the authors sample production rules from an error correcting grammar. While theoretically sound, this technique is incomplete, i.e., does not guarantee finding a small repair, if one exists. In this paper, we demonstrate it is possible to attain a significant advantage by synthesizing and scoring all repairs within a certain Levenshtein distance. Not only does this technique guarantee completeness and perfect length-generalization, it also aids precision when ranking by naturalness.

  We take a first-principles approach making no assumptions about the sentence or grammar and focus on correctness and end-to-end latency. Our technique is simple:

  \begin{enumerate}
    \item We model syntax correction as a language intersection problem between a Levenshtein automaton and a context-free grammar, which we explicitly materialize using a specialized version of the Bar-Hillel construction to Levenshtein intersections.
    \item We construct a data structure via idempotent matrix completion that compactly represents the intersection language. This data structure is used to index valid parse forests and significantly reduce relevant productions from the intersection grammar.
  ...
  Though simple, this technique outperforms state-of-the-art syntax repair techniques. Its efficacy owes to the fact it does not sample edits or nor productions, but unique, fully formed repairs within a certain Levenshtein distance. It is sound and complete up to a Levenshtein bound - i.e., it will find all repairs within an arbitrary Levenshtein distance, and no more. Often, the language of small repairs is surprisingly small relative to the language of all possible edits, enabling us to efficiently synthesize and score every possible solution via enumeration. Enumeration confers a significant advantage over memoryless sampling techniques, allowing us to synthesize the most natural repair with far less data and compute than would otherwise be required by a large language model.
