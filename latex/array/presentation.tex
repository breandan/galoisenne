...
P = 21 = 1  0  1  0  1          a  b  c  d  e     s0
 ...
 "taps" ─┴─────┴─────╯                M        *  S
    ...
    \begin{frame}[fragile]
        ...
        \begin{tabular}{ c l{0.4\textwidth}}
        \begin{tikzpicture}
        ...
        \end{scope}
            \end{tikzpicture} & \begin{itemize}
                                             \item Suppose we wanted to sample n-grams.\\
                                             \item LFSRs sample without replacement from indexed families $S_V$ in $\mathcal{O}(\log |S|)$ space and $\mathcal{O}(1)$ time.\\
                                             \item Suppose we had a rank-n tensor. We could keep track of the visited entries manually (expensive). Or, we can form a bijection with GF(2^n)\\
        \end{itemize}\\
        \end{tabular}
    \egroup
    \end{frame}
...
%            \item Characteristic polynomials, companion matrices, eigenvalues
            ...
            \item Many problems in PL and graph theory reducible to rootfinding
...
%            \item What is the meaning of abstract algebraic eigenvalues?
        ...
        Types
        \begin{itemize}
            \item Implement a typelevel PRNG
        \end{itemize}
        Probability
        \begin{itemize}
