...
%            \item Set joins are an active topic of research in SQL query optimization
    ...
    \begin{frame}[fragile]{A type family for graphs}
        \begin{align*}
           \textsc{Vertex} &\rightarrow \textsc{Int}\\
           \textsc{Neighbors} &\rightarrow \textsc{Vertex} \mid \textsc{Vertex} \phantom{\vee} \textsc{Neighbors} \\
           \textsc{Context} &\rightarrow ([\textsc{Neighbors}], \phantom{\vee} \textsc{Vertex}, \phantom{\vee} [\textsc{Neighbors}]) \\
           \textsc{Graph} &\rightarrow \textsc{Empty} \mid \textsc{Context} \phantom{\vee} \textsc{Graph}
        \end{align*}
        ...
        interface IGF<G, E, V> where
          G: IGraph<G, E, V>, E: IEdge<G, E, V>, V: IVertex<G, E, V> {
            ...
            fun G(l: List<Any>): G = when {
              ...
              l allAre l.first() -> list.map { it as V }.toSet()
            }.let { G(it) }
          }

        interface IGraph<G, E, V>: IGF<G, E, V> /* ... */ {
            operator fun plus(that: G): G =
              G((this - that) + (this join that) + (that - this))
            operator fun minus(graph: G): G = G(vertices - graph.vertices)
            infix fun join(that: G): Set<V> = TODO("Exercise for readers")
        }
        \end{lstlisting}
    \end{frame}
