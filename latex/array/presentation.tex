...
%            \item Set joins are an active topic of research in SQL query optimization
    ...
    \begin{frame}
        ...
        Selecting any $V \neq \mathbf{0}$ and coefficients $P_j$ from a \textit{primitive polynomial} generates an ergodic sequence over GF$(2^n)$ with full periodicity: $\mathbf{V} = \begin{pmatrix}V & \mathbf{M}V & \mathbf{M}^{2}V & \cdots & \mathbf{M}^{2^n-1}V \end{pmatrix}$.
    \end{frame}

    \begin{frame}[fragile]
        \frametitle{Linear finite state registers}
        \begin{verbatim}
P = 21 = 1  0  1  0  1          a  b  c  d  e     s0
         ∥  ∥  ∥  ∥  ∥          1  0  0  0  0     s1
         a  b  c  d  e          0  1  0  0  0  *  s2
   ╭──── ⊕ ─── ⊕ ────╮          0  0  1  0  0     s3
   │     │     │     │          0  0  0  1  0     s4
   1 ──► 0  1  0  1  1 ──►
 "taps" ─┴─────┴─────╯                M        *  S
    ...
    \begin{frame}[fragile]
        \frametitle{Multidimensional sampling}
        Can sample without replacement from indexed families $S_V$ in $\mathcal{O}(\log |S|)$ space and $\mathcal{O}(1)$ time.

        Suppose we had a rank-n tensor. We could keep track of the visited and unvisted entries manually. However this would be expensive. Instead, we can establish a bijection with GF(2^n)!
    ...
    \begin{frame}[fragile]
        ...
        fun List<Int>.toBitLens(): List<Int> =
          map { ceil(log2(it.toDouble())).toInt() }
    ...
    \begin{frame}[fragile]
        ...
        fun Sequence<List<Boolean>>.hastyPudding(lengths: List<Int>) =
          map { it.toIndexes(lengths.toBitLens()) }
           .filter { it.zip(lengths).all { (a, b) -> a < b } }

        // If the dimensions all share the same coordinate set
        fun <T> MDSamplerNoReplacement(set: Set<T>, dim: Int = 1) =
          MDSamplerWithoutReplacement(List(dim){ set })

        fun <T> MDSamplerNoReplacement(
          dimensions: List<Set<T>>, // If the dimensions are different
          lengths: List<Int> = dimensions.map { it.size },
          // Shuffle coordinates to increase entropy of sampling
          shuffDims: List<List<T>> = dimensions.map { it.shuffled() },
          bitLens: List<Int> = dimensions.map(Set<T>::size).toBitLens(),
          degree: Int = bitLens.sum().also { println("LFSR(GF(2^$it))") }
        ): Sequence<List<T>> =
          LFSR(degree).map { it.toBitList(degree) }.hastyPudding(lenghts)
            .map { shuffDims.zip(it).map { (dims, idx) -> dims[idx] } } +
            sequenceOf(shuffDims.map { it[0] })
        \end{lstlisting}
    \end{frame}
    ...
    \begin{frame}
        ...
        \begin{itemize}
            ...
            \item Tradition starting with Euler through Galois to Kantorovich
            ...
            \item These forms are *much faster* than powering the matrix iteratively
            \item Many problems in PL theory and graph theory reducible to rootfinding
            \item Context free parsing is just rootfinding on a semiring algebra
            \item Subtyping sans recursive types is just graph reachability
            ...
            \item By doing so, we can exploit well-known algebraic techniques
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}
        ...
        \item Extend this work to context-sensitive grammars
        \item What is the connection to Leibnizian differentiability?
        \item Can we interpret gradients and derivatives?
        \item What is the meaning of abstract algebraic eigenvalues?
        \item Look into probabilistic grammars, e.g., PCFGs
        \item Diffusion models have a very natural interpretation on graphs
        \item Look into optimization (e.g., LP/MILP/QP) to rank feasible set
        \end{itemize}
    \end{frame}
