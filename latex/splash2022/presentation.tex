  ...
  \begin{figure}[H]
    \adjustbox{scale=0.75,center}{%
      \[\begin{tikzcd}[row sep=large, col sep=huge]
          ...
          \highlight{\Sigma}^{n-1} \arrow[dr] && \mathcal{G}_\varepsilon \arrow[dl]\arrow[ddl, bend left] \arrow[dr]\arrow[ddr, bend right] & & \err{\Sigma^{n-1}} \arrow[dl] \\
          & V^{n-1} \arrow[d] & & (V_\varepsilon \cup \{\_\})^{n-1} \arrow[d, shift left]\arrow[d, shift right] & \\
          & \mathbb{Z}_2^{n\times n \times |V|} \arrow[d, shift left] \arrow[d, shift right] & & \mathbb{Z}^{n\times n \times |V_\varepsilon|}_2 \rightarrow \mathbb{Z}_2^{|V_{\varepsilon}|} \arrow[d, shift left]\arrow[d, shift right] & \\
          & CST & & \langle \Sigma \setminus \{\varepsilon\}, CST \rangle &
      \end{tikzcd}\]
    }
  \end{figure}
  Our algorithm produces set of concrete syntax trees (CSTs) for a given valid string. Otherwise if the string contains an error, our algorithm returns a set of admissible corrections, alongside their CSTs.
\end{frame}
...
\begin{frame}[fragile]{Error correction: Known Error Locations}
  \begin{figure}[H]
    \hspace{-0.25cm}
    \begin{tikzpicture}
      \begin{axis}[
...
\begin{frame}[fragile]{Error correction: Unknown Error Locations}
  \begin{figure}[H]
    \hspace{-0.25cm}
    \begin{tikzpicture}
      \begin{axis}[
...
\begin{frame}[fragile]{Error correction: MiniGithub Java Dataset}
  \begin{figure}[H]
...
\begin{frame}[fragile]{Error correction: Break-it-fix-it Dataset}
  \begin{figure}[H]
    \hspace{-0.25cm}
    \begin{tikzpicture}
      \begin{axis}
...
\begin{frame}{Linear Finite State Registers}
  ...
  \[
    \mathbf{S} = \begin{pmatrix}
                   V & \mathbf{M}V & \mathbf{M}^{2}V & \mathbf{M}^{3}V & \cdots & \mathbf{M}^{2^n-1}V
    \end{pmatrix}
  \]
