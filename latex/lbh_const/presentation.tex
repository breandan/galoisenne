...
\section{Examples and Motivation}\label{sec:ex_motiv}
...
\noindent\phantom{...}\begin{tabular}{|c|c|c|c|}
  \hline
  \phantom{$M_\infty$:} & $2^V$ & $\mathbb{Z}_2^{|V|}$ & $\mathbb{Z}_2^{|V|}\rightarrow\mathbb{Z}_2^{|V|}$\\\hline
  $M_0$: & \begin{pmatrix}
  \phantom{V} & \tiny{\{N\}} &         &             \\
              &              & \{N,O\} &             \\
              &              &         & \{N,O\} \\
...
\begin{frame}[t,fragile]{The Bar-Hillel construction and its specialization}
  ...
  \noindent\begin{prooftree}
             ...
             \AxiomC{$\vphantom{\highlight{(}}p,q,r \in Q$}
             \RightLabel{$\Join$}
...
\section{Bar-Hillel Refinements}\label{sec:bh-refinements}

\begin{frame}[fragile]{Edit location refinement: intuition}
  Let's think: do we really need $|\sigma| \times d_\max$ states? Can we somehow narrow down the edit range? Where can we remove dead weight?
...
\begin{frame}[t,fragile]{Edit location refinement: example}
  \begin{onlyenv}<1>
      Let's prune states absorbing impossible repair trajectories!\vspace{-0.5cm}
    \begin{figure}[H]
    \resizebox{\textwidth}{!}{
      \begin{tikzpicture}[
%->, % makes the edges directed
...
%        \draw [decorate,decoration={brace,amplitude=10pt,raise=20pt}] (00.south west) -- (0k.north west) node[midway,xshift=-1cm,yshift=-1cm,rotate=-54]{\textbf{Edit distance}};
  ...
  \end{onlyenv}
  \begin{onlyenv}<3>
  Let's prune states absorbing impossible repair trajectories!\vspace{-0.675cm}
  \begin{figure}[H]
    \resizebox{\textwidth}{!}{
      \begin{tikzpicture}[
%->, % makes the edges directed
...
\section{Left-to-right Decoding}\label{sec:decoding}
...
\begin{frame}[t,fragile]{So, we have a grammar. Now what?}
  ...
  \only<1>{Now, let's step back and think\ldots\\}
  \only<2->{Now, let's step back and think\ldots the intersection language is finite\ldots\\}
  \only<3->{So we should be able to represent it as a DFA.}
\end{frame}

\begin{frame}[fragile]{Potential ambiguity of Levenshtein-Bar-Hillel grammars}
The previous technique produces a grammar that exactly generates $G_\cap$, but does not guarantee string uniqueness since the CFG may be ambiguous.
...
We can eliminate ambiguity and thereby improve the rate of convergence for natural syntax repair by first translating $G_\cap$ into an FSA.
\end{frame}
...
Let $+, \cdot: \mathcal{A}\times \mathcal{A} \rightarrow \mathcal{A}$ be automata operators satisfying the property $\mathcal{L}(A_1 + A_2) = \mathcal{L}(A_1)\cup\mathcal{L}(A_2)$, and $\mathcal{L}(A_1 \cdot A_2) = \mathcal{L}(A_1)\times\mathcal{L}(A_2)$. We can translate $G_\cap$ to $\mathcal{A}$, as follows, recalling FSAs are closed over $+, \cdot$:

\begin{equation*}
\mathcal{Y}(v: V) \mapsto \begin{cases}
\alpha \mid \mathcal{L}(\alpha) = \{s \in \Sigma_\cap \mid v \rightarrow s\} & \textit{ if v is a unit NT}, \\
\sum_{\langle v_1, v_2\rangle \mid v \rightarrow v_1 v_2} \mathcal{Y}(v_1)\cdot\mathcal{Y}(v_2) & \text{ otherwise.}
\end{cases}
\end{equation*}
%
...
\begin{frame}[fragile]{Results from uniform sample efficiency experiments}
\begin{figure}[h!]
...
\vspace{-0.5cm}
\caption{Sankey diagram of 5136 repair instances sampled uniformly from the StackOverflow Python dataset balanced acrost ($\lfloor|\err\sigma| / 10\rfloor \in [0, 8], \Delta(\err\sigma, \sigma') < 4$). (ER=Error, NR=Not recognized, NG=Not generated). Time: $\sim$10hr on M1 Max.}
\end{figure}
\end{frame}
